...
if (!isset($StockID)) {
	...
	if (!isset($OrderNumber) OR $OrderNumber==''){
		...
		echo '<table class="selection">
			<tr>
				...
				<td><select name="StockLocation"   id="StockLocation"   onChange="onStockCat( this )"> ';
		...
		$sql = "SELECT locations.loccode,
				...
				INNER JOIN locationusers ON locationusers.loccode=locations.loccode AND locationusers.userid='" .  $_SESSION['UserID'] . "' AND locationusers.canview=1
				 AND locations.loccode IN (SELECT categoryid	FROM stockcategory WHERE stocktype='M')";
   ...
   /*
	$SQL="SELECT categoryid,
			categorydescription
		FROM stockcategory WHERE stocktype='M' AND categoryid IN (SELECT loccode FROM locationusers WHERE  userid='".$_SESSION['UserID']."')
	...
	echo '<br />
	   ...
	   foreach( $StockCategory as $row){
		   ...
		   if ($row['loc']==$_POST['StockLocation']){
...
//end if stock search results to show
	...
	//Harmonize the ordervalue with SUM function since webERP allowed same items appeared several times in one sales orders. If there is no sum value, this situation not inclued.
			...
			$SQL = "SELECT salesorders.orderno,
					...
					SUM(salesorderdetails.currprice*(salesorderdetails.quantity-salesorderdetails.qtyinvoiced)) AS currordervalue,
					SUM(salesorderdetails.taxprice*(salesorderdetails.quantity-salesorderdetails.qtyinvoiced)) AS ordervalue
				FROM salesorders 
				INNER JOIN salesorderdetails ON salesorders.orderno = salesorderdetails.orderno					
					LEFT JOIN registername ON registername.regid=salesorders.debtorno					
					WHERE salesorderdetails.completed=0  
					AND salesorders.debtorno IN (SELECT `regid` FROM `customerusers` WHERE userid='".$_SESSION['UserID']."') ";
	...
	if (DB_num_rows($SalesOrdersResult)>0) {
		...
		if (isset($_POST['Quotations']) AND $_POST['Quotations']=='Orders_Only'){
			$TableHeader = '<tr>
								...
								<th class="ascending" >' . _('Order Total') . '<br />外币</th>
		...
		while ($myrow=DB_fetch_array($SalesOrdersResult)) {
			...
			$FormatedCurrOrderValue = locale_number_format($myrow['currordervalue'],$_SESSION['CompanyRecord']['decimalplaces']);
				...
				if ($AuthRow['cancreate']==0 AND $myrow['poplaced']==0){ //cancreate==0 if the user can create POs and not already placed
					printf('<td><a href="%s">%s</a></td>
						...
						$Confirm_Invoice,
		 				$PrintDispatchNote,		 				
						 $myrow['brname'],
						 $_SESSION['CompanyRecord'][ $myrow['tag']]['unitstab'],
		 				$myrow['customerref'],
		 				$FormatedOrderDate,
		 				$FormatedDelDate,
		 				html_entity_decode($myrow['deliverto'],ENT_QUOTES,'UTF-8'),
						 $FormatedCurrOrderValue.' '.$myrow['currcode'],
						 $FormatedOrderValue,
				   $i,
				   $i,
				   $myrow['orderno']);
		...
		echo '<tfoot><tr><td colspan="10" class="number">';
		if ($_POST['Quotations']=='Orders_Only'){
			echo '<b>' . _('Total Order(s) Value in');
		} else {
			echo '<b>' . _('Total Quotation(s) Value in');
		}
		echo ' ' . $_SESSION['CompanyRecord'][$myrow['tag']]['currencydefault'] . ' :</b></td>
			<td class="number"><b>' . locale_number_format($OrdersTotal,$_SESSION['CompanyRecord'][$myrow['tag']]['decimalplaces']) . '</b></td>
			</tr></tfoot>
			</table>';
