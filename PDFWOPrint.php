...
if (isset($SelectedWO) AND $SelectedWO != '' AND $SelectedWO > 0 AND $SelectedWO != 'Preview') {
	...
	$sql = "SELECT workorders.wono,
						...
						FROM workorders 
						INNER JOIN woitems
						ON workorders.wono=woitems.wono
...
/* Load the relevant xml file */
 // $Mak==77;
//if ($Mak==6) {
if (isset($MakePDFThenDisplayIt) || isset($MakePDFThenEmailIt)) {
	...
	if ($MakePDFThenDisplayIt) {
		...
		if($_SESSION['SmtpSetting'] == 0){
			...
			prnMsg('499='.$_SESSION['CompanyRecord']['email'] .	$Success); 
