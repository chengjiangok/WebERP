...
//new number must be shown
		...
		if ($_SESSION['AutoCode'] != $_POST['X_AutoCode']){
			$sql[] = "UPDATE config SET confvalue='" . $_POST['X_AutoCode'] . "' WHERE confname='AutoCode'";
		}
		if ($_SESSION['Tag'] != $_POST['X_Tag']){
			$sql[] = "UPDATE config SET confvalue='" . $_POST['X_Tag'] . "' WHERE confname='Tag'";
		}
		if ($_SESSION['Audit'] != $_POST['X_Audit']){
			$sql[] = "UPDATE config SET confvalue='" . $_POST['X_Audit'] . "' WHERE confname='Audit'";
		}
		if ($_SESSION['Currency'] != $_POST['X_Currency']){
			$sql[] = "UPDATE config SET confvalue='" . $_POST['X_Currency'] . "' WHERE confname='Currency'";
		}
		...
		//	prnMsg("UPDATE config SET confvalue = '". $_POST['X_CountryOfOperation'] ."' WHERE confname = 'CountryOfOperation'");
		}
		//prnMsg("UPDATE config SET confvalue = '". $_POST['X_CountryOfOperation'] ."' WHERE confname = 'CountryOfOperation'");
...
echo '<tr style="outline: 1px solid"><td>供应商编号方式:</td>
...
echo '<tr style="outline: 1px solid"><td>物料编号方式:</td>
	<td><select name="X_AutoCode">';

if ($_SESSION['AutoCode']==0) {
	echo '<option selected="selected" value="0">' . _('Manual Entry') . '</option>';
	echo '<option value="1">' . _('Automatic') . '</option>';
} else {
	echo '<option selected="selected" value="1">' . _('Automatic') . '</option>';
...
echo '</select></td>
	<td>物料编号方式，设置为手动可以自动编号或填入号码</td></tr>';



//==HJ== drop down list for tax category
$sql = "SELECT `taxid` taxcatid, `description` taxcatname,taxrate  FROM `taxauthorities` WHERE onorder IN (1,3) ORDER BY description";
//SELECT taxcatid, taxcatname FROM taxcategories ORDER BY taxcatname";
...
*/
echo '<tr style="outline: 1px solid"><td>账务分组设置:</td>
	<td><select name="X_Tag">';

if ($_SESSION['Tag']==0) {
	echo '<option selected="selected" value="0">单分组</option>';
	echo '<option value="1">多分组</option>';
} else {
	echo '<option selected="selected" value="1">多分组</option>';
	echo '<option value="0">单分组</option>';
}
echo '</select></td>
	<td>设置多分组方式，可以在一个账套核算多套账务、并自动分立或合并。</td></tr>';
	echo '<tr style="outline: 1px solid"><td>会计凭证审核设置:</td>
	<td><select name="X_Audit">';

if ($_SESSION['Audit']==0) {
	echo '<option selected="selected" value="0">不审核</option>';
	echo '<option value="1">审核</option>';
} else {
	echo '<option selected="selected" value="1">审核</option>';
	echo '<option value="0">不审核</option>';
}
echo '</select></td>
	<td>会计凭证审核设置，设置为不审核,可以打印、查询、结账,设置为审核,打印、查询、结账必须先审核。</td></tr>';
echo '<tr style="outline: 1px solid"><td>多币种设置:</td>
	   <td><select name="X_Currency">';
if ($_SESSION['Currency']==0) {
		echo '<option selected="selected" value="0">本币</option>';
		echo '<option value="1">多币种</option>';
	} else {
		echo '<option selected="selected" value="1">多币种</option>';
		echo '<option value="0">本币</option>';
	}
	echo '</select></td>
		<td>多币种设置，设置为多币种,可以进行多币种核算,设置为本币、只能核算本币。</td></tr>';
