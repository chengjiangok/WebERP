/*
 * @Author: ChengJiang 
 * @Date: 2017-03-06 11:49:56 
 * @Last Modified by: ChengJiang
 * @Last Modified time: 2017-03-07 20:09:04
 */
<?php
/* $Id: GLTrialBalanceTag.php 7268 2016-09-08 14:57:47Z ChengJiang $*/
...
if (!isset($_POST['prdrange']) OR $_POST['prdrange']==''){
	...
	if (isset($_POST['prdrange'])){
		//PageSet('PD',$_POST['prdrange']);		
  		$pdend=$_POST['prdrange'];
	}	else {
	 	$pdend=0;
	}
	$prdrange= $_SESSION['period'];
	if (isset($_POST['tag'])){
	...
	*/
	...
	echo '</select></td><td>'.'起'.'';
	  echo '<select name="prdrange" size="1" >';
    for( $i=0; $i <= 12; $i++) {    	
    	if(	$_POST['prdrange']== $i ){
		  echo '<option selected="selected" value ="'.$i.'" >' .$i.'个月'. '</option>';		 
			 	
		} elseif ($i==0 OR $i==3 OR $i==6 OR $i==12 ) {
    ...
    echo'</select>
     	  ...
     	  $sql="SELECT tagID, tagdatabase,tagdescription FROM unittag  ORDER BY tagID ";
        $result = DB_query($sql);  
     echo '<tr>
     	    <td>单元分组</td>
  			<td><select name="unittag" size="1" >';
  		$k=0;
  	if(isset($_POST['unittag'])==50) {
  	 echo '<option selected="selected"   value="50">' ._('All') . '</option>';
  	}	
  ...
  while ($myrow=DB_fetch_array($result,$db)){
   ...
   if (isset($_SESSION['Tag'])){
    ...
    $result = DB_query($sql);
	 ...
	 echo '<option   value="0">' ._('Share') . '</option>';
...
/*echo '		<tr>
			<td>' . '查询方式'. '</td>
				<td colspan="2"><input type="radio" name="chtype" value="3">'._(All).'          
            <input type="radio" name="chtype" value="2">'._(Total).'
            <input type="radio" name="chtype" value="1" checked>'._(Detail).' </td> </tr>';*/
  /*	echo '<tr><td>' . "选择分组") . ':</td>
	...
	if (file_exists( $_SESSION['reports_dir'] . '/VT.csv')){
		...
		while ($mytype = fgetcsv($FileVT)) { 
			 	if( $mytype[2]== PageGet('TG','50') AND $k==0){
			 		...
			 		echo '<option  selected="selected"  value="' . $mytype[2] . '">' .  $mytype[1]) . '</option>';
			 	  echo '<option   value="50">' .  "全部") . '</option>';
			 		$k=1;
			 	}else{
			 		if ( 50== PageGet('TG','50') AND $k==0 ){
			 			echo '<option selected="selected" value="50">' .  "全部") . '</option>';
			 			$k=1;
			 		}
			 			echo '<option  value="' . $mytype[2] . '">' .  $mytype[1]) . '</option>';
 ...
 /*   $sql="SELECT code,description FROM  workcentres";
    ...
    if (file_exists( $_SESSION['reports_dir'] . '/workcenter.csv')){
		...
		while ($myrow = fgetcsv($FileVT)) { 
			 	if( $myrow[2]== PageGet('HS','0') AND $k==0 ){
			 		...
			 		echo '<option  selected="selected"  value="' . $myrow[2] . '">' .  $myrow[1]) . '</option>';
			 		...
			 		if ( 0== PageGet('TG','0') AND $k==0 ){
			 			...
			 			echo '<option  value="' . $myrow[2] . '">' .  $myrow[1]) . '</option>';
	...
	*/
	...
	echo '<div class="centre">
			...
			<input type="submit" name="PrintPDF" value="'.'创建PDF'.'" />
		</div>';
...
include('includes/tcpdf/tcpdfAccountBalance.php');
	...
	$BalanceDate = $myrow[0];
	$SQL = "call accountbalance('".$_POST['selectprd']."','" .$chtype."')";
...
//============================================================+
 ...
 // prnMsg("'".$perno."','" .$prdrange."','".$chtype."','".$tag."','".$hstag."'",'info');
...
/*
    ...
    echo $TableHeader;
	while ($myrow=DB_fetch_array($AccountsResult)) {
		$ActEnquiryURL = '<a href="'. $RootPath . '/GLAccountInquiry.php?FromPeriod=' . $_POST['selectprd'] . '&amp;selectprd=' . $_POST['selectprd'] . '&amp;Account=' . $myrow['account'] . '&amp;Show=Yes">' . $myrow['account'] . '</a>';
      $j=$j+1;
  //if ($j<$k){
  	if ($k==1){
			echo '<tr class="EvenTableRows">';
...
*/
	 ...
	 prnMsg('该功能模块暂时未开通！'.'chtype'.$chtype.'$tag'.$tag.'hstag'.$hstag,'info');
}
echo '</div>
	</form>';
include('includes/footer.php');
