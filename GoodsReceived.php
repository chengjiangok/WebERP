...
/* $Id: GoodsReceived.php  $*/
/*
 * @Author: ChengJiang 
 * @Date: 2018-10-13 14:33:31 
 * @Last Modified by: ChengJiang
 * @Last Modified time: 2018-10-13 14:36:23
 */
...
/*The identifier makes this goFods received session unique so cannot get confused
...
//只有当用户点击按钮刷新数据并重新计算商品的值时，才会发生更新。
    $_SESSION['PO'.$identifier]->OrderNo=$_GET['PONumber'];
...
if ($_SESSION['PO'.$identifier]->SomethingReceived()==0 AND isset($_POST['ProcessGoodsReceived'])){ /*Then dont bother proceeding cos nothing to do ! */
	...
	while ($myrow = DB_fetch_array($Result)) {
		...
		if ($_SESSION['PO'.$identifier]->LineItems[$LineNo]->GLCode != $myrow['glcode'] OR
			...
			prnMsg($_SESSION['PO'.$identifier]->LineItems[$LineNo]->GLCode .'!='. $myrow['glcode'] .'<br>'.
					$_SESSION['PO'.$identifier]->LineItems[$LineNo]->ShiptRef .'!='.$myrow['shiptref'] .'<br>'.
					$_SESSION['PO'.$identifier]->LineItems[$LineNo]->JobRef .'!='.$myrow['jobref'] .'<br>'.
					$_SESSION['PO'.$identifier]->LineItems[$LineNo]->QtyInv .'!='.$myrow['qtyinvoiced'] .'<br>'.
					$_SESSION['PO'.$identifier]->LineItems[$LineNo]->StockID .'!='.$myrow['itemcode'] .'<br>'.
					$_SESSION['PO'.$identifier]->LineItems[$LineNo]->Quantity .'!='.$myrow['quantityord'] .'<br>'.
					$_SESSION['PO'.$identifier]->LineItems[$LineNo]->QtyReceived .'!='.$myrow['quantityrecd'].'<br>');

			prnMsg('384-'._('This order has been changed or invoiced since this delivery was started to be actioned') . '. ' . _('Processing halted') . '. ' . _('To enter a delivery against this purchase order') . ', ' . _('it must be re-selected and re-read again to update the changes made by the other user'),'warn');
			...
			// Insert stock movements - with unit cost //
				...
				$SQL = "INSERT INTO stockmoves (stockid,
									...
									VALUES (
										...
										'" . $OrderLine->ReceiveQty . "',
										'" . round($_SESSION['PO'.$identifier]->LineItems[$OrderLine->LineNo]->StandardCost ,2). "',
