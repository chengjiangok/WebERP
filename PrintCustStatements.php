 ...
 * @LastEditTime: 2020-04-06 06:42:47
 */
...
/*Figure out who all the customers in this range are */
	...
	if (DB_Num_Rows($StatementResults) == 0){
		...
		prnMsg('90'. _('There were no Customers matching your selection of '). $_POST['FromCust']. ' - '.
			$_POST['ToCust'].'.' , 'error');
 ...
 */
	...
	if (isset($pdf) AND $_POST['EmailOrPrint']=='print'){
			$pdf->OutputD($_SESSION['CompanyRecord'][$_SESSION['Tag']]['coyname'] . '_CustStatements_' . date('Y-m-d') . '.pdf');
			$pdf->__destruct();
	} elseif (!isset($pdf)) {
	...
	/*if FromTransNo is not set then show a form to allow input of either a single statement number or a range of statements to be printed. Also get the last statement number created to show the user where the current range is up to */
		...
		echo '<tr>
				...
				<td><input type="text" maxlength="10" size="8" name="FromCust" value="1" /></td></tr>
			<tr>
				<td>' ,  _('Ending Customer statement to print (Customer code)') , '</td>
				<td><input type="text" maxlength="10" size="8" name="ToCust" value="9999" /></td>
			</tr>
			<tr>
				<td>选择打印/邮件</td>
				...
				<br /><div class="centre">
				<input type="submit" name="PrintPDF" value="打印或发送邮件报表" />
			</div>';
        echo '</div>
              </form>';
	}
