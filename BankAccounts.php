...
/*
 * @Author: ChengJiang 
 * @Date: 2017-06-11 10:14:14 
 * @Last Modified by: ChengJiang
 * @Last Modified time: 2018-03-03 11:08:17
 */
...
echo '<div class="page_help_text">银行账户维护简介: 设置货币币种,银行账户及账号,设置对账单格式</div><br />';
...
if (isset($SelectedBankAccount) AND !isset($_GET['delete'])) {
 ...
 $sql="SELECT accountcode,
				accountname
			FROM chartmaster 
			WHERE  accountcode like '100%' and length(accountcode)>4 and accountcode not in (SELECT accountcode FROM bankaccounts)
...
echo '<tr>
	...
	<tr>
		...
		$result = DB_query("SELECT bankformat.bankid,bank_name  FROM bankformat LEFT JOIN bankname ON bankname.bankid=bankformat.bankid");

		while ($myrow = DB_fetch_array($result)) {
			if ($myrow['bankid']==$_POST['ImportFormat']) {
				echo '<option selected="selected" value="'.$myrow['bankid'] . '">' . $myrow['bank_name'] . '</option>';
			} else {
				echo '<option value="'.$myrow['bankid'] . '">' . $myrow['bank_name'] . '</option>';
			}
		} //end while loop
	echo'</select>
		</td>
