...
<?php
/* $Id: CustomerBranches.php  ChengJiang$*/
/*
 * @Author: ChengJiang 
 * @Date: 2018-11-28 09:55:59 
 * @Last Modified by: ChengJiang
 * @Last Modified time: 2018-11-29 08:20:42
 */
...
if (isset($_SESSION['Tag']) && $_SESSION['Tag']>0){
		...
		$sql="SELECT branchcode, debtorno FROM custbranch WHERE debtorno='".$DebtorNo."'  AND branchcode LIKE  '".$_POST['unittagID']."%' ORDER BY branchcode DESC";
		$ErrMsg = _('The stock categories could not be retrieved because');
		$DbgMsg = _('The SQL used to retrieve stock categories and failed was');
		$result = DB_query($sql,$ErrMsg,$DbgMsg);
		//prnMsg($sql,'info');
    $myrow=DB_fetch_assoc($result);
    if (empty($myrow['branchcode'])){
	   	$_POST['BranchCode']=$_POST['unittagID'].'1';
    }else{
		$_POST['BranchCode']=$_POST['unittagID'].($myrow['branchcode']+1);
 	}
  		
}else{
	$sql="SELECT branchcode, debtorno FROM custbranch WHERE debtorno='".$DebtorNo."'  ORDER BY branchcode DESC";
		$ErrMsg = _('The stock categories could not be retrieved because');
		$DbgMsg = _('The SQL used to retrieve stock categories and failed was');
		$result = DB_query($sql,$ErrMsg,$DbgMsg);
		//prnMsg($sql,'info');
    ...
    if (empty($myrow['branchcode'])){
		  ...
		  $_POST['BranchCode']=$myrow['branchcode']+1;
 	}
	...
	if (isset($SelectedBranch) AND $InputError !=1) {
		...
		$SQL = "UPDATE custbranch SET brname = '" . $_POST['BrName'] . "',
						...
						email='" . $_POST['Email'] . "',
						taxcatid='" . explode('^',$_POST['TaxGroup'])[0] . "',
						taxrate='" . explode('^',$_POST['TaxGroup'])[1] . "',
						taxgroupid='" . explode('^',$_POST['TaxGroup'])[2] . "',
	...
	/*Selected branch is null cos no item selected on first time round so must be adding a	record must be submitting new entries in the new Customer Branches form */
		...
		$SQL = "INSERT INTO custbranch (branchcode,
				...
				VALUES ('" . $_POST['BranchCode'] . "',
					...
					'" . $_POST['Area'] . "',
					'" . $_POST['Email'] . "',
					'" . explode('^',$_POST['TaxGroup'])[0] . "',
					'" . explode('^',$_POST['TaxGroup'])[1] . "',
					'" . explode('^',$_POST['TaxGroup'])[2] . "',
	...
	if ($result){
	$SQL="INSERT INTO custsupusers(	code,
	                                    csno,									
										userid,
										notype,
										canview,
...
if (!isset($SelectedBranch)){
	...
	$SQL = "SELECT debtorsmaster.name,
				...
				ON custbranch.taxcatid=taxauthorities.taxid
				WHERE custbranch.debtorno = '".$DebtorNo."'";

	if ($_SESSION['SalesmanLogin'] != '') {
		$SQL .= " AND custbranch.salesman='" . $_SESSION['SalesmanLogin'] . "'";
	}
     //prnMsg($myrow['branchcode'][0].$SQL,'info');
	$result = DB_query($SQL);
	$myrow = DB_fetch_row($result);
	$TotalEnable = 0;
	$TotalDisable = 0;
...
if (!isset($_GET['delete'])) {
	...
	*/
	...
	if (isset($_SESSION['Tag']) AND $_SESSION['Tag']>0 AND !isset($DebtorNo)){
		echo '<tr>
		<td>' . _('Units Tag') . ':</td>
		<td><select name="unittagID" onchange="ReloadForm(ItemForm.UpdateCategories)">';

		$sql="SELECT concat(LEFT(code,1),tagID) unittagID,concat(description,'-', tagdescription) description 
        FROM unittag, workcentres  WHERE flag=0";
		$ErrMsg = _('The stock categories could not be retrieved because');
		$DbgMsg = _('The SQL used to retrieve stock categories and failed was');
		$result = DB_query($sql,$ErrMsg,$DbgMsg);

		while ($myrow=DB_fetch_array($result)){
			if (!isset($_POST['unittagID']) OR  $myrow['unittagID']==$_POST['unittagID']){
				echo '<option selected="selected" value="'. $myrow['unittagID'] . '">' . $myrow['description'] . '</option>';
			} else {
				echo '<option value="'. $myrow['unittagID'] . '">' . $myrow['description'] . '</option>';
			}
			$Category=$myrow['unittagID'];
		}

		if (!isset($_POST['unittagID'])) {
			$_POST['unittagID']=$Category;
		}
	...
	//$SQL = "SELECT a.taxcatid,taxauthority,taxcatname, taxrate FROM taxcategories a LEFT JOIN taxauthrates b ON a.taxcatid=b.taxcatid";
		//SELECT taxgroupid, description FROM taxgroups";
	...
	while ($myrow = DB_fetch_array($TaxGroupResults)) {
		if (isset($_POST['TaxGroup']) AND $myrow['taxid'].'^'.$myrow['taxrate']==$_POST['TaxGroup']) {
			echo '<option selected="selected" value="';
		} else {
			echo '<option value="';
		}
	...
	echo '<tr>
		...
		<br />
		<div class="centre">
			<input name="submit" tabindex="29" type="submit" value="保存或更新" />
		</div>
		</div>
		</form>';
