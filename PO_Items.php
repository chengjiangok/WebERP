 ...
 * @LastEditTime: 2020-04-06 15:47:43
 */
...
echo'<script type="text/javascript">
	function inTaxCat(s,d,r) {
		var x = s.selectedIndex; // 选中索引
		var val =s.options[x].value
		var rate=val.split("^")[1];
		var taxid=val.split("^")[0];
		alert(rate);				
	}	
	function inPrice(p,d,r){		
		...
		if (vlqty.value!=""){
			...
			document.getElementById("Price"+n).value=(parseFloat(p.value)/(1+parseFloat(val))).toFixed(2);
			document.getElementById("TaxAmo"+n).value=(total*val).toFixed(2);
		}
		
		var taxtol=(val*total).toFixed(2); 
		document.getElementById("TaxAmo"+n).value=taxtol;
		document.getElementById("Amount"+n).value=(Number(taxtol)+Number(total)).toFixed(2); 
		var taxtotal=0;
		var amototal=0;
		for(var i=1; i<=r; i++){
	...
	function inQTY(p,d,r){
		...
		if (vl.value!=""){
			...
			document.getElementById("Amount"+n).value=total;
			document.getElementById("Price"+n).value=(parseFloat(vl.value)/(1+parseFloat(val))).toFixed(2);
	...
	function inAmount(p,d,r){
		...
		if (qty==0){
			...
			document.getElementById("SuppQty"+n).value=qty;
			alert("你输入数字小数位数和设置不同,系统自动按设置计算,默认"+d+"位!");
		}	
		var vlprice=0;			
		var obj = document.getElementById("TaxCat"+n); 
		...
		if (vlqty.value!=""){
			...
			document.getElementById("SuPrice"+n).value=vlprice;		
			document.getElementById("Price"+n).value=(parseFloat(vlprice)/(1+parseFloat(val))).toFixed(2);
			document.getElementById("TaxAmo"+n).value=(parseFloat(p.value)/(1+val)*val).toFixed(2);
		}
		var taxtotal=0;
		var amototal=0;
		for(var i=1; i<=r; i++){
...
if (isset($_POST['UpdateLines']) OR isset($_POST['Commit'])) {
	...
	foreach ($_SESSION['PO'.$identifier]->LineItems as $POLine) {
		if ($POLine->Deleted == false) {
			...
			if (!is_numeric(filter_number_format($_POST['SuPrice'.$POLine->LineNo]))){
				...
				$_SESSION['PO'.$identifier]->LineItems[$POLine->LineNo]->Price = filter_number_format($_POST['SuPrice'.$POLine->LineNo])/$_SESSION['PO'.$identifier]->LineItems[$POLine->LineNo]->ConversionFactor;
			}
    ...
    foreach ($_SESSION['PO'.$identifier]->LineItems as $POLine) {
		//prnMsg($_POST['edit'.$POLine->LineNo]);
	  ...
	  //prnMsg(explode('^',$_POST['taxauth'.$POLine->LineNo])[1].'-'.$_POST['SupTax'.$POLine->LineNo]);
	}
...
if (isset($_POST['Commit'])){ /*User wishes to commit the order to the database */
	...
	if ($_SESSION['PO'.$identifier]->DelAdd1=='' or mb_strlen($_SESSION['PO'.$identifier]->DelAdd1)<3){
		prnMsg('60'.$_SESSION['PO'.$identifier]->DelAdd1. _('The purchase order cannot be committed to the database because there is no delivery street address specified'),'error');
		$InputError=1;
	} else*/if ($_SESSION['PO'.$identifier]->Location=='' or ! isset($_SESSION['PO'.$identifier]->Location)){
 ...
 */
	if ($InputError!=1){
		...
		if ($_SESSION['AutoAuthorisePO']==1) {
			//如果用户有权授权采购订单，则将自动授权if the user has authority to authorise the PO then it will automatically be authorised
				...
				if ($POLine->Deleted==true) {
					if ($POLine->PODetailRec!='') {
						 ...
						 * So it will only be a new POLine if PODetailRec is empty
						 当采购订单行现有记录自动递增字段 PODetailrec给会话，
						 POLine会是一个新的PODetailrec POLine如果是空的
						*/
					$sql = "INSERT INTO purchorderdetails ( orderno,
														itemcode,
														deliverydate,
...
if (count($_SESSION['PO'.$identifier]->LineItems)>0 and !isset($_GET['Edit'])){
	...
	$TaxSql="SELECT `taxcatid`, `taxcatname`, `taxrate` FROM `taxcategories` WHERE onorder in (2,3)";
	...
	foreach ($_SESSION['PO'.$identifier]->LineItems as $POLine) {
		...
		if ($POLine->Deleted==False) {
			...
			if ($POLine->Price > 1) {
				...
				$SuppPrice = locale_number_format(round($POLine->Price,$_SESSION['PO'.$identifier]->CurrDecimalPlaces),$_SESSION['PO'.$identifier]->CurrDecimalPlaces);
				$Price = locale_number_format(round($POLine->Price/(1+$_SESSION['PO'.$identifier]->TaxRate),$_SESSION['PO'.$identifier]->CurrDecimalPlaces),$_SESSION['PO'.$identifier]->CurrDecimalPlaces);

			} else {
				$DisplayPrice = locale_number_format($POLine->Price,4);
				$SuppPrice = locale_number_format(round($POLine->Price,4),4);
				$Price = locale_number_format(round($POLine->Price/(1+$_SESSION['PO'.$identifier]->TaxRate),4),4);
			}
			$TaxAmo =round($LineTotal/(1+$_SESSION['PO'.$identifier]->TaxRate),$_SESSION['PO'.$identifier]->CurrDecimalPlaces);
			...
			echo '<td>' . $POLine->StockID  . '</td>
				...
				<input type="hidden" id="edit' . $POLine->LineNo . '" name="edit' . $POLine->LineNo . '" value="0"> </td>';
				//<td class="number">' . locale_number_format($POLine->Quantity,$POLine->DecimalPlaces) . '</td>
			echo'<td>' . $POLine->Units . '</td>';
				...
				*/
			echo'<td><input type="text" class="number" id="SuppQty' . $POLine->LineNo .'" name="SuppQty' . $POLine->LineNo .'" size="10" onChange="inQTY(this,'.$POLine->DecimalPlaces .' ,'.$rw.' )" value="' . locale_number_format(round($POLine->Quantity,$POLine->DecimalPlaces),$POLine->DecimalPlaces) . '"  pattern="(^-?\d{1,10})(.\d{0,'.$POLine->DecimalPlaces.'})?$"　  title="匹配浮点数！"  /></td>';
			//	<td>' . $POLine->SuppliersUnit . '</td>
			echo'<td><input type="text" class="number" id="SuPrice' . $POLine->LineNo . '" name="SuPrice' . $POLine->LineNo . '" size="10"  onChange="inPrice(this,'.$POLine->DecimalPlaces .','.$rw.' )"  value="' . $SuppPrice .'"   title="匹配浮点数！"  /></td>';
			echo'<td><select name="TaxCat' . $POLine->LineNo .'"  id="TaxCat' . $POLine->LineNo .'"  onChange="inTaxCat(this,'.$POLine->DecimalPlaces .','.$rw.' )" disabled="disabled">';
			...
			echo '</select></td>';
			echo'<td><input type="text" class="number" name="Price' . $POLine->LineNo . '" id="Price' . $POLine->LineNo . '" size="10" value="' . $Price .'"  readonly="readonly" /></td>';
			echo'<td><input type="text" class="number" name="TaxAmo' . $POLine->LineNo . '" id="TaxAmo' . $POLine->LineNo . '" size="10" value="' . locale_number_format( $TaxAmo,$_SESSION['PO'.$identifier]->CurrDecimalPlaces) .'"  readonly="readonly" /></td>';
			
			echo'<td><input type="text" class="number" name="Amount' . $POLine->LineNo . '" id="Amount' . $POLine->LineNo . '"  size="10"  onChange="inAmount(this,'.$POLine->DecimalPlaces .','.$rw.' )"   value="' . $LineTotal .'" /></td>';
			echo'<td><input type="text" class="date" alt="' .$_SESSION['DefaultDateFormat'].'" name="ReqDelDate' . $POLine->LineNo.'" size="10" value="' .$POLine->ReqDelDate .'" /></td>';
	...
	echo '<tr><td colspan="7" class="number">' . _('TOTAL') . '</td>
			  <td><input type="text"  class="number"   id="TaxTotal" maxlength="10" size="10" value="'. locale_number_format($TaxAmoTotal,$_SESSION['PO'.$identifier]->CurrDecimalPlaces). '" readonly="readonly" /></td>
		      <td><input type="text"  class="number"   id="AmountTotal" maxlength="10" size="10" value="'. $DisplayTotal. '" readonly="readonly" /></td>
			  <td colspan="2"></td>
