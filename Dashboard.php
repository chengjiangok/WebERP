...
/*
 * @Author: ChengJiang 
 * @Date: 2019-03-24 08:42:31 
 * @Last Modified by:   ChengJiang 
 * @Last Modified time: 2019-03-24 08:42:31 
 */
...
echo '<p class="page_title_text"><img src="'.$RootPath.'/css/'.$Theme.'/images/gl.png" title="' . $Title . '" alt="" />' . $Title . '</p>';
	//$sql="select * from informantion as a where infodate = (select max(b.infodate) from informantion as b where a.infotype = b.infotype )";
	$sql="SELECT infotype, MAX(infodate) infodate, content FROM informantion GROUP BY infotype";
	...
	while ($myrow=DB_fetch_array($result,$db)){
		$msg.=$x.'.'.$myrow['content'].'-'.$myrow['infodate'].'</br>';
		$x++;
	}
	
		$sql="SELECT   sum( CASE WHEN  printno <= 0  THEN 1 ELSE 0 END ) AS flg0 ,
					sum(CASE WHEN prtchk=0  THEN 1 ELSE 0 END) AS flg2 FROM gltrans WHERE  gltrans.periodno='".$_SESSION['period'] ."' ";
	$result = DB_query($sql);
	...
	$sql="SELECT account ,SUM( amount) AS amount 
				FROM gltrans 
				WHERE periodno>=0  AND periodno<='".$_SESSION['period']."' AND (account LIKE '1601%' OR account LIKE '1602%') GROUP BY account";
      ...
      if (DB_fetch_row($result)>0){
        ...
        $i=0;
      	while ($myrow=DB_fetch_array($result,$db)){
					...
					$fixgltn[$myrow['account']]	=$myrow['amount'];	
...
echo '<td><div style="width:500px;height:'.$hg.'px;background-color: #ECFFFF;"> ';
	$sql = "SELECT *		FROM www_users	WHERE www_users.userid='" . $_SESSION['UserID']  . "'";
	$result = DB_query($sql);
  $myrow = DB_fetch_array($result);

if ($_SESSION['AccessLevel']==8 OR $_SESSION['AccessLevel']==6 OR $_SESSION['AccessLevel']==13 OR $_SESSION['AccessLevel']==12){			
	echo $msg;
		   $zero1=strtotime (date('y-m-d')); //当前时间
			 $zero2=strtotime ($_SESSION['lastdate']);  //时间
		if (($zero1-$zero2)>=86400) {
			 $zz=$zero1-$zero2;
			 echo $x.'.'.'当前会计期间和当前日期不匹配！</br>';//.date('y-m-d h:d:s',$zero1).'-'.date('y-m-d h:d:s',$zero2).'='.$zz;
			 $x++;
	 }
}		
if ($_SESSION['AccessLevel']==8 OR $_SESSION['AccessLevel']==6 OR $_SESSION['AccessLevel']==12){			

		if  ($prtchk >0 ) {
			echo $x.'.'.'当前会计期间的会计分录,没有打印：'.$prtchk.' ';
		 
		}
		if  ($prtchk2 >0 ) {
			echo '没有审核：'.$prtchk2.'</br>';
		}
	$i++;
}	
	echo '</div></td>	</tr>'					;
	...
	if ($_SESSION['AccessLevel']==8 OR $_SESSION['AccessLevel']==6 OR $_SESSION['AccessLevel']==12 OR $_SESSION['AccessLevel']==13){
		...
		$dt1=date('Y-m-d',strtotime('-'.$week.'day'));
		$dt2=date('Y-m-d');
		$sql = " SELECT account ,
					bankaccountname,
				SUM(CASE WHEN periodno>=0 AND trandate <'".$dt1."' THEN amount ELSE 0 END) qcye, 
				SUM(CASE WHEN ((amount>0 AND flg=1) OR (amount<0 AND flg=-1)) AND trandate>='".$dt1."' AND trandate <= '".$dt2."' THEN amount ELSE 0 END)  AS Debit,
				SUM(CASE WHEN ((amount<0 AND flg=1) OR (amount>0 AND flg=-1)) AND trandate>='".$dt1."' AND trandate <= '".$dt2."' THEN -amount ELSE 0 END)  AS Credit ,
				SUM(CASE WHEN periodno>=0 AND trandate <= '".$dt2."' THEN amount ELSE 0 END) qmye
				FROM `gltrans` 
				JOIN bankaccounts ON bankaccounts.accountcode=gltrans.account
				WHERE periodno>=0 AND account IN (SELECT `accountcode` FROM `bankaccountusers` WHERE userid='".$_SESSION['UserID']."' ) GROUP BY account ";
		...
		if (DB_num_rows($result1)>0){
			echo '<table class="selection"><tbody>';		
			echo '<tr>
					<th colspan="5">银行现金账户信息<br>' . $dt1.'-'.$dt2 . '</th>
				</tr>';
			echo '<tr>
					<th class="ascending">' . _('GL Account') . '</th>
					<th class="ascending">期初余额</th>					
					<th class="ascending">借方发生</th>
					<th class="ascending">贷方发生</th>
					<th class="ascending">期末余额</th>
				</tr>';					
			...
			while ($myrow = DB_fetch_array($result1)) {
				...
				printf('<td>%s</td>										
						...
						$myrow['account'] . ' - ' . $myrow['bankaccountname'],
					
						locale_number_format(($myrow['qcye']),$_SESSION['CompanyRecord']['decimalplaces']),
						locale_number_format(($myrow['Debit']),$_SESSION['CompanyRecord']['decimalplaces']),
						locale_number_format(($myrow['Credit']),$_SESSION['CompanyRecord']['decimalplaces']),
						locale_number_format(($myrow['qmye']),$_SESSION['CompanyRecord']['decimalplaces']));
			} //each bank account
			echo '</tbody>
					</table>';
		}
...
if (in_array($CashSecurity, $_SESSION['AllowedPageSecurityTokens']) OR !isset($CashSecurity)) {
	...
	$sql = " SELECT account ,bankaccountname,SUM(CASE WHEN periodno>=0 AND trandate <'".$dt1."' THEN amount ELSE 0 END) qcye, 
					SUM(CASE WHEN ((amount>0 AND flg=1) OR (amount<0 AND flg=-1)) AND trandate>='".$dt1."' AND trandate <= '".$dt2."' THEN amount ELSE 0 END)  AS Debit,
					SUM(CASE WHEN ((amount<0 AND flg=1) OR (amount>0 AND flg=-1)) AND trandate>='".$dt1."' AND trandate <= '".$dt2."' THEN -amount ELSE 0 END)  AS Credit ,
					SUM(CASE WHEN periodno>=0 AND trandate <= '".$dt2."' THEN amount ELSE 0 END) qmye FROM `gltrans` 
					JOIN bankaccounts ON bankaccounts.accountcode=gltrans.account
					WHERE periodno>=0 AND account IN (SELECT `accountcode` FROM `bankaccountusers` WHERE userid='".$_SESSION['UserID']."' ) GROUP BY account ";
	$ErrMsg = _('The bank accounts set up could not be retrieved because');
	$DbgMsg = _('The SQL used to retrieve the bank account details was') . '<br />' . $sql;
