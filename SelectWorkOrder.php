...
/* $Id: SelectWorkOrder.php $*/
/*
 * @Author: ChengJiang 
 * @Date: 2018-09-20 09:28:35 
 * @Last Modified by: ChengJiang
 * @Last Modified time: 2018-10-28 19:55:33
 */
	...
	if (!isset($SelectedWO) or ($SelectedWO=='')){
		...
		/*SELECT locations.loccode,
				...
				INNER JOIN locationusers 
					...
					INNER JOIN stockcategory ON stockcategory.categoryid=locations.loccode 
				WHERE locations.usedforwo = 1 AND stockcategory.stocktype='M'";*/
...
/*
	$SQL="SELECT categoryid,
	          categorydescription
			FROM stockcategory
			LEFT JOIN locationusers ON locationusers.loccode = categoryid AND locationusers.userid = '".$_SESSION['UserID']."' AND locationusers.canupd = 1
			WHERE stocktype = 'M'
	...
	*/
	...
	while ($myrow = DB_fetch_array($result1)) {
		if ($myrow['loccode'] == $_POST['StockCat']){
			echo '<option selected="selected" value="' . $myrow['loccode'] . '">' . $myrow['locationname'] . '</option>';
	   } else {
			echo '<option value="' . $myrow['loccode'] . '">' . $myrow['locationname'] . '</option>';
	   }
	...
	if (isset($StockItemsResult)) {
		...
		if (isset($SelectedWO) AND $SelectedWO !='') {
						...
						$SQL.=" AND workorders.wo='". $SelectedWO ."'  AND
							  workorders.closed IN (" . $ClosedOrOpen . ")
							  AND stockmaster.categoryid='" . $_POST['StockLocation'] . "'";
			...
			//	INNER JOIN locationusers ON locationusers.loccode=workorders.loccode  AND locationusers.userid='" .  $_SESSION['UserID'] . "'AND locationusers.canview=1
			  ...
			  /* $DateAfterCriteria = FormatDateforSQL($OrdersAfterDate); */
				...
				if (isset($SelectedStockItem)) {
					$SQL .= " AND  woitems.stockid='". $SelectedStockItem ."'
							AND  workorders.closed IN (" . $ClosedOrOpen . ")
						AND stockmaster.categoryid='" . $_POST['StockLocation'] . "'
							";
			//	INNER JOIN locationusers ON locationusers.loccode=workorders.loccode AND locationusers.userid='" .  $_SESSION['UserID'] . "' AND locationusers.canview=1
								 
				} else {
					$SQL .= "AND workorders.closed IN (" . $ClosedOrOpen . ")
							AND stockmaster.categoryid='" . $_POST['StockLocation'] . "'		";
									 //
									 //	INNER JOIN locationusers ON locationusers.loccode=workorders.loccode AND locationusers.userid='" .  $_SESSION['UserID'] . "' AND locationusers.canview=1
				}
		} //end not order number selected
		$SQL.="	ORDER BY woitems.wo,
				woitems.stockid";
       // prnMsg($SQL);
		$ErrMsg = _('No works orders were returned by the SQL because');
		$WorkOrdersResult = DB_query($SQL,$ErrMsg);

		/*show a table of the orders returned by the SQL	<th>' . _('Costing') . '</th> */
		...
		while ($myrow=DB_fetch_array($WorkOrdersResult)) {
            ...
            if ($myrow['qtyrecd']>0){
				$ModifyPage =$myrow['wo'].'-'.$r;
			}else{
				if ($myrow['closed']==0){
					$ModifyPage = '<a href="'.$RootPath . '/WorkOrderEntry.php?wo=' . $myrow['wo'].'-'.$myrow['wo'].'">'.$myrow['wo'].'-'.$r.'</a>';
				}else{
					$ModifyPage =$myrow['wo'].'-'.$r;
				}
			}
			if ($myrow['closed']==0){
				$Status_WO ='<a href="'.$RootPath . '/WorkOrderStatus.php?WO=' .$myrow['wo'] . '&amp;StockID=' . $myrow['stockid'].'">' . _('Status') . '</a>';
			}else{
				$Status_WO ="关闭发料";
			}
			if ($myrow['closed']==2){
				...
				$Receive_WO ='<a href="'. $RootPath . '/WorkOrderReceive.php?WONO=' .$myrow['wo']. '-'.$myrow['wo'] . '&amp;StockID=' . $myrow['stockid'].'">收货</a>';
			}
			if ($myrow['closed']==0){
				$Issue_WO = '<a href="'.$RootPath . '/WorkOrderIssue.php?WONO=' .$myrow['wo'] . '-'.$myrow['wo'].'&amp;StockID=' . $myrow['stockid']. '">发料</a>';
			}else{
				$Issue_WO="关闭发料";
			}//$Costing_WO =$RootPath . '/WorkOrderCosting.php?WONO=' .$myrow['wo']. '-'.$myrow['wo'];
			$Printing_WO =$RootPath . '/PDFWOPrint.php?WONO=' .$myrow['wo'] . '-'.$myrow['wo']. '&amp;StockID=' . $myrow['stockid'];
