...
/*
 * @Author: ChengJiang 
 * @Date: 2018-12-16 18:52:22 
 * @Last Modified by: ChengJiang
 * @Last Modified time: 2018-12-16 19:02:12
 */
include('includes/session.php');

if (isset($_POST['Jnl'])) {
	$str=url_decode($_POST['Jnl']);
	prnMsg($str);
	$str=explode('^',$str);
}else if (isset($_GET['Jnl'])) {
	$str=url_decode($_GET['Jnl']);
	prnMsg($str);
	$str=explode('^',$str);
...
include('includes/tcpdf/PDFJournal.php');
	...
	$sql="SELECT gltrans.typeno,
				...
				WHERE gltrans.periodno='".$periodno."' 	AND gltrans.transno='" . $Jnl . "'			
...
// END OF FILE
		ob_end_clean();
			...
			$pdffilename=$ym.'会计凭证打印号'.$Jnl.'.pdf';
		
			$pdf->Output($pdffilename,'D');
			$pdf->__destruct();

			$sql="update  gltrans set printno=abs(printno) where  periodno=".$periodno." and abs(printno)=".$Jnl." printno<0";
