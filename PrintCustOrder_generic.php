...
If (!isset($_GET['TransNo']) OR $_GET['TransNo']==""){
	...
	prnMsg('15'. _('Select an Order Number to Print before calling this page') , 'error');
	echo '<br />
			<br />
			<br />
...
$sql = "SELECT salesorders.debtorno,
		...
		LEFT JOIN unittag ON unittag.tagID = salesorders.tag
		
			WHERE salesorders.orderno='" . $_GET['TransNo'] . "'";
			//INNER JOIN locationusers ON locationusers.loccode=locations.loccode AND locationusers.userid='" .  $_SESSION['UserID'] . "' AND locationusers.canview=1
	
		//INNER JOIN locations 		ON salesorders.fromstkloc=locations.loccodelocations.locationname,
...
if (DB_num_rows($result)==0){
	...
	include('includes/header.php');
	echo '<div class="centre"><br /><br /><br />';
	prnMsg( '89'._('Unable to Locate Order Number') . ' : ' . $_GET['TransNo'] . ' ', 'error');
	echo '<br />
			<br />
			<br />
			<table class="table_index">
	...
	exit();
        ...
        // Place the deliver blind variable into a hold variable to used when      producing the packlist 
        ...
        if ($myrow['printedpackingslip']==1 AND ($_GET['Reprint']!='OK' OR !isset($_GET['Reprint']))){
              ...
              //echo '<p><a href="' . $RootPath . '/PrintCustOrder.php?TransNo=' . $_GET['TransNo'] . '&Reprint=OK">'	. _('Do a Re-Print') . ' (' . _('On Pre-Printed Stationery') . ') ' . _('Even Though Previously Printed') . '</a><p>' ;
				echo '<br /><br />';
               echo '<a href="' . $RootPath. '/PrintCustOrder_generic.php?TransNo=' . $_GET['TransNo'] . '&Reprint=OK">' .  _('Do a Re-Print') . ' - 发货单</a>';
...
// Then there's an order to print and its not been printed already (or its been flagged for reprinting) LETS GO 
...
/*for ($i=1;$i<=1;$i++){  //Print it out twice one copy for customer and one for office 
	//
	if ($i==2){
		$PageNumber = 1;
		$pdf->newPage();
	...
	$sql = "SELECT salesorderdetails.stkcode,
				...
				AND locstock.loccode=stockmaster.categoryid";
				// locstock.loccode = '" . $myrow['fromstkloc'] . "'
		//prnMsg($sql);		
	$result=DB_query($sql, $ErrMsg);
	...
	if (DB_num_rows($result)>0){
		...
		while ($myrow2=DB_fetch_array($result)){
			...
			$LeftOvers = $pdf->addTextWrap($XPos,$YPos,50,$FontSize,$i,'center',1);
			$LeftOvers = $pdf->addTextWrap(100,$YPos,270,$FontSize,$myrow2['stkcode'].' '.$myrow2['description'],'left',1);
			//$LeftOvers = $pdf->addTextWrap(220,$YPos,150,$FontSize,$myrow2['description'],'right',1);
			$LeftOvers = $pdf->addTextWrap(370,$YPos,80,$FontSize,$DisplayQty,'right',1);
			$LeftOvers = $pdf->addTextWrap(450,$YPos,100,$FontSize,'','right',1);
			
			 // $LeftOvers = $pdf->addTextWrap(550,$YPos,80,$FontSize,'','right',1);
			//$LeftOvers = $pdf->addTextWrap(670,$YPos,80,$FontSize,$DisplayPrevDel,'right',1);
            $i++;
			if ($YPos-$line_height <= 50){
				$YPos-=5;
				$LeftOvers = $pdf->line($XPos,$YPos,550,$YPos);
				
				$LeftOvers =$pdf->addText($XPos+50, $YPos,$FontSize,'验收人:' );
				$LeftOvers =$pdf->addText($XPos+200, $YPos,$FontSize,  '交物人:' );
				$LeftOvers =$pdf->addText($XPos+350, $YPos,$FontSize,  '制单人:');
...
if ($ListCount == 0) {
	...
	ob_end_clean();
	$pdf->OutputD('销售发货单_' .$_GET['TransNo'].'_'. date('Ymd') . '.pdf');
	$pdf->__destruct();
