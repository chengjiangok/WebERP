...
function ShowDays(&$db)  {//####LISTALL_LISTALL_LISTALL_LISTALL_LISTALL_LISTALL_LISTALL_####
	...
	while ($myrow = DB_fetch_array($result)) {
		...
		if ($ctr==0){
		 ...
		 if($myrow[3]=='Sunday'){
			echo'<td>'.ConvertSQLDate($myrow[0]).'</td></tr>';
		 }else{
			 echo'<td>'.ConvertSQLDate($myrow[0]).'</td>';
		 }
		}
		 $ctr++;
...
function ShowDays1(&$db)  {//####LISTALL_LISTALL_LISTALL_LISTALL_LISTALL_LISTALL_LISTALL_####
...
function ShowInputForm1(&$db,&$ChangeDate)  {//####DISPLAY_DISPLAY_DISPLAY_DISPLAY_DISPLAY_DISPLAY_#####

	// Display form fields. This function is called the first time
	// the page is called, and is also invoked at the end of all of the other functions.
	
		if (!isset($_POST['FromDate'])) {
			$_POST['FromDate']=date('Y-m-01',strtotime(PeriodGetDate($_SESSION['janr'])));
...
// the page is called, and is also invoked at the end of all of the other functions.
	...
	if (!isset($_POST['FromDate'])) {
		$_POST['FromDate']=date('Y-m-01',strtotime(PeriodGetDate($_SESSION['janr'])));
		$_POST['ToDate']=date('Y-12-31',strtotime($_SESSION['lastdate']));
	}
	echo '<form action="' . htmlspecialchars($_SERVER['PHP_SELF'],ENT_QUOTES,'UTF-8') . '" method="post">
          <div>
			<br />
