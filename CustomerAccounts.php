...
if (!isset($_POST['selectprd'])){
	...
	$msg.="仓储物料最末单��日期:".$stockdate['trandate'].'<br>';
	//读取未核对材料的凭证
	...
	$sql="SELECT periodno, lastdate_in_period FROM periods WHERE periodno>='".$_SESSION['janr']."' AND  periodno<='". $_SESSION['period']."'";
	$result = DB_query($sql);
	...
	echo '<tr>
			...
			if ($row==0){
				...
				while ($myrow=DB_fetch_array($result,$db)){
					...
					echo $myrow['periodno'] .'^'.$myrow['lastdate_in_period'] . '">' .$myrow['lastdate_in_period']  . '</option>'; 
				}
			}
			echo'</td>
				  </tr>';
...
if (isset($_POST['SearchSettle'])&& $_POST['Select']=="") {
	$SQL="SELECT `invno`, `invtype`, `tag`, `transno`, `period`, `invdate`, `amount`, `tax`, `currcode`, `toregisterno`, `toaccount`, `toname`, `tobank`, `toaddress`, `stockname`, `spec`, `unit`, `price`, `quantity`, `remark`, `flg`, `stled` FROM `invoicetrans` WHERE period='".$_SESSIEN['period']."' AND invtype=0";
	...
	$sql="SELECT 
				...
				WHERE a.type=10 AND trandate<='".explode("^",$_POST['selectprd'])[1]."'
						AND stled=0 ";
	...
	while($row=DB_fetch_array($result)){
		...
		echo '<td>'.$RowIndex.'</td>
		        <td><a href="#">' .$RowIndex. $row['suppname']. '</a></td>
						
				<td ">'.$row['description'].'</td>
				<td class="number">'.locale_number_format($row['amount'],POI).'</td>			
				<td class="number">'.locale_number_format($row['qty'],POI).'</td>
				<td class="number">'.locale_number_format($row['amount'],POI).'</td>
				<td class="number"></td>
	...
	$sql="SELECT  a.connectid ,
			...
			WHERE a.type IN (17,25) AND trandate<='".explode("^",$_POST['selectprd'])[1]."'
