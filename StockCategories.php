...
<?php
/* $Id: StockCategories.php  $*/
/*
 * @Author: ChengJiang 
 * @Date: 2017-08-21 20:45:43 
 * @Last Modified by:   ChengJiang 
 * @Last Modified time: 2017-08-21 20:45:43 
 */
...
$StockTypeName = array();
$StockTypeName['D'] = _('Dummy Item - (No Movements)');//虚拟（无动作）
$StockTypeName['M'] = _('Finished Goods');//成品
$StockTypeName['L'] = _('Labour');//劳动
$StockTypeName['B'] = _('Raw Materials');//原材料
asort($StockTypeName);
...
$Query = "SELECT `taxid`, `description`, `taxrate` FROM `taxauthorities` WHERE onorder IN (2,3)";
$Result = DB_query($Query);
while ($Row = DB_fetch_array($Result)) {
	$TaxCategoryName[$Row['taxid']] = $Row['description'];
}*/
...
if (isset($_POST['submit'])) {
	...
	if (isset($SelectedCategory) AND $InputError !=1) {
		...
		$sql = "UPDATE stockcategory SET stocktype = '" . $_POST['StockType'] . "',
									 ...
									 taxcatact = '" . $_POST['TaxCatAct'] . "',
									 stockact = '" . $_POST['StockAct'] . "',
									 adjglact = '" . $_POST['AdjGLAct'] . "',
									 issueglact = '" . $_POST['IssueGLAct'] . "',
									 labourcode = '" . $_POST['LabourCode'] . "',
									 overheadcode = '" . $_POST['OverheadCode'] . "',
									 purchpricecode = '" . $_POST['PurchPriceCode'] . "',
									 adjglcode = '" . $_POST['AdjGLCode'] . "',	
									 labouract = '" . $_POST['LabourAct'] . "',
									 overheadact = '" . $_POST['OverheadAct'] . "',							 
									 wipact = '" . $_POST['WIPAct'] . "'
									 WHERE
	...
	unset($_POST['PurchPriceCode']);
	unset($_POST['AdjGLCode']);
	unset($_POST['WIPAct']);
	...
	echo '<br />
		<table class="selection">
			<tr>
				...
				<th class="ascending">税种科目</th>' . '
				<th class="ascending">' . _('Stock GL') . '</th>' . '
				<th class="ascending">' . _('Adjts GL') . '</th>
				<th class="ascending">盘点差异编码</th>
				<th class="ascending">物料差异科目</th>
				<th class="ascending">物料差异编码</th>
				<th class="ascending">人工费科目</th>
				<th class="ascending">人工费编码</th>
				<th class="ascending">制造费用科目</th>
				<th class="ascending">制造费用编码</th>
...
if (isset($SelectedCategory)) {
	...
	if (!isset($_POST['UpdateTypes'])) {
		...
		$_POST['PurchPriceCode']  = $myrow['purchpricecode'];
		$_POST['AdjGLCode']  = $myrow['adjglcode'];
		$_POST['LabourCode']  = $myrow['labourcode'];
		$_POST['LabourAct']  = $myrow['labouract'];
		$_POST['OverheadCode']  = $myrow['overheadcode'];
		$_POST['OverheadAct']  = $myrow['overheadact'];
		$_POST['WIPAct']  = $myrow['wipact'];
		$_POST['TaxCatAct']  = $myrow['taxcatact'];
	}
	...
	echo '<table class="selection">
			<tr>
				<td>' . _('Category Code') . ':</td>
				<td><input type="text" name="CategoryID" required="required" autofocus="autofocus" data-type="no-illegal-chars" title="' . _('Enter up to six alphanumeric characters or underscore as a code for this stock category') . '" size="12" maxlength="6" value="' . $_POST['CategoryID'] . '" /></td>
			</tr>';
}
...
$sql = "SELECT accountcode,
			...
			WHERE left(accountcode,4) in( '1403','1405','5001') AND LENGTH(accountcode)>4
...
$sql = "SELECT accountcode,
			...
			LEFT JOIN accountgroups
				ON chartmaster.group_=accountgroups.groupname
				WHERE left(accountcode,3) IN ( '140','640','500') AND LENGTH(accountcode)>4
			ORDER BY accountcode";
...
// Stock Type input.
echo '<tr>
		<td><label for="StockType">' . _('Stock Type') .':</label></td>
...
echo '</select>
	  ...
	  $sql = "SELECT accountcode,
				...
				LEFT JOIN accountgroups
					ON chartmaster.group_=accountgroups.groupname
				WHERE left(accountcode,6)='222101' AND LENGTH(accountcode)>4
				ORDER BY accountcode";

$Result = DB_query($sql);
// Default Tax Category input.
/*
if (!isset($_POST['TaxCatAct'])) {
	$_POST['TaxCatAct'] = $_SESSION['DefaultTaxCategory'];
}*/
echo '<tr>
		 <td><label for="TaxCatAct">' . _('Default Tax Category') .':</label></td>
		 <td><select id="TaxCatAct"  name="TaxCatAct">';
while ($Row=DB_fetch_array($Result)) {
	echo '<option';
	if ($_POST['TaxCatAct'] == $Row['accountcode']) {
		echo ' selected="selected"';
	}
	echo ' value="' .  $Row['accountcode']. '">' . $Row['accountname'] . '</option>';
}
echo '</select>
      </td></tr>';
...
// WIP GL Code input.
echo '<tr><td>' . _('WIP GL Code') . ':</td>
	  <td><select name="WIPAct">';
	  $r=0;
while ($myrow = DB_fetch_array($PnLAccountsResult)) {
	if (substr($myrow['accountcode'],0,3)=='500'){
		$r++;
...
while ($myrow = DB_fetch_array($PnLAccountsResult)) {
	if (substr(trim($myrow['accountcode']),0,3)=='140'){
...
echo'<tr>
		<td>盘点差异编码:</td>
		<td><input type="text" name="AdjGLCode"  data-type="no-illegal-chars" title="" size="12" maxlength="20" value="' . $_POST['AgjGLCode'] . '" />
		</td>
	</tr>';
echo'<tr>
	<td>物料差异科目:</td>
...
echo'<tr>
		<td>物料差异编码:</td>
		<td><input type="text" name="PurchPriceCode"  data-type="no-illegal-chars" title="" size="12" maxlength="20" value="' . $_POST['PurchPriceCode'] . '" />
		</td>
	</tr>';
	DB_data_seek($PnLAccountsResult,0);
echo '<tr>
		<td>人工费成本:</td>
		<td><select name="LabourAct">';

while ($myrow = DB_fetch_array($PnLAccountsResult)) {
	if (substr(trim($myrow['accountcode']),0,3)=='500'){
		if (isset($_POST['LabourAct']) and $myrow['accountcode']==$_POST['LabourAct']) {
			echo '<option selected="selected" value="' . $myrow['accountcode'] . '">' . htmlspecialchars($myrow['accountname'], ENT_QUOTES, 'UTF-8', false) . ' ('.$myrow['accountcode'].')' . '</option>';
		} else {
			echo '<option value="' . $myrow['accountcode'] . '">' . htmlspecialchars($myrow['accountname'], ENT_QUOTES, 'UTF-8', false) . ' ('.$myrow['accountcode'].')' . '</option>';
		}
...
echo'<tr>
		<td>人工费编码:</td>
		<td><input type="text" name="LabourCode"  data-type="no-illegal-chars" title="" size="12" maxlength="20" value="' . $_POST['LabourCode'] . '" />
		</td>
	</tr>';
	DB_data_seek($PnLAccountsResult,0);
echo '<tr>
		<td>制造费用成本:</td>
		<td><select name="OverheadAct">';

while ($myrow = DB_fetch_array($PnLAccountsResult)) {
	if (substr(trim($myrow['accountcode']),0,3)=='500'){
		if (isset($_POST['OverheadAct']) and $myrow['accountcode']==$_POST['OverheadAct']) {
			echo '<option selected="selected" value="' . $myrow['accountcode'] . '">' . htmlspecialchars($myrow['accountname'], ENT_QUOTES, 'UTF-8', false) . ' ('.$myrow['accountcode'].')' . '</option>';
		} else {
			echo '<option value="' . $myrow['accountcode'] . '">' . htmlspecialchars($myrow['accountname'], ENT_QUOTES, 'UTF-8', false) . ' ('.$myrow['accountcode'].')' . '</option>';
		}
...
echo'<tr>
		<td>制造费用编码:</td>
		<td><input type="text" name="OverheadCode"  data-type="no-illegal-chars" title="" size="12" maxlength="20" value="' . $_POST['OverheadCode'] . '" />
		</td>
	</tr>';
