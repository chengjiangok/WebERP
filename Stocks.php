...
<?php
/* $Id: Stocks.php ChengJiang  $ */
/*
 * @Author: ChengJiang 
 * @Date: 2017-03-03 11:23:13 
 * @Last Modified by: mikey.zhaopeng
 * @Last Modified time: 2019-08-14 09:13:00
 */
...
if (isset($_GET['StockID'])){
	...
	/*	$sql="SELECT subcategoryid, categoryid, stocktype, code, subcategorydspn, flg
		FROM stocksubcategory WHERE subcategoryid =LEFT('".$StockID."',length(subcategoryid))";
	$result=DB_query($sql);
...
//prnMsgprnMsg( $_POST['CategoryID'].'-'.$_GET['StockID'],'info');
...
if (isset($_POST['submit'])) {
    //prnMsg($StockID.'-]'.$_SESSION['AutoCode']);
	...
	//自动物料号
	$SQL="SELECT  confvalue FROM myconfig WHERE  confname='StockNumber' ";
	$Result = DB_query($SQL);
	
	$row = DB_fetch_assoc($Result);				
	$NoRule=explode(',',$row['confvalue']);
	$_POST['CategoryID']=explode('^',$_POST['SubCategory'])[0];
	if (!isset($_GET['StockID']) && ($StockID==''||(!isset($StockID)||empty($StockID)))){
		if(isset($_POST['subStockid']) && strlen($_POST['subStockid'])>=strlen($NoRule[1])){
			$_POST['StockID']=explode('^',$_POST['SubCategory'])[1].$_POST['subStockid'];
			$StockID=$_POST['StockID'];
		}else{		
			$_POST['StockID']=CreateCode(explode('^',$_POST['SubCategory'])[1],$NoRule);
			$StockID=$_POST['StockID'];
		}
			//prnMsg($StockID.'='.$_POST['SubCategory'].','.strlen($NoRule[0]));
		
		/*elseif(isset($_SESSION['AutoCode'])){
			
				//自动产生编码
				$sql="SELECT MAX(stockid) stockid, categoryid FROM stockmaster WHERE stockid LIKE '".explode('^',$_POST['SubCategory'])[1]."%' ORDER BY stockid DESC";
				$result = DB_query($sql);
				$myrow = DB_fetch_row($result);
				if (!empty($myrow)){
					$stockstr=str_replace(explode('^',$_POST['SubCategory'])[1],'',$myrow[0]);
					$StockID=explode('^',$_POST['SubCategory'])[1].str_pad(($stockstr+1),strlen($stockstr),'0',STR_PAD_LEFT);
					$_POST['StockID']=$StockID;
				
				}else{
					$StockID=explode('^',$_POST['SubCategory'])[1].'001';  
					$_POST['StockID']=$StockID;
			...
			$_POST['CategoryID']=explode('^',$_POST['SubCategory'])[0];
			//prnMsg($myrow['stockid'][0].'=$StockID'.$StockID,'info');
	...
	if (isset($_SESSION['AutoCode']) &&($_SESSION['AutoCode']==1 )){
			$codelen=$NoRule[2];//编码长度
			$subws=strlen($NoRule[1]);//编码长度
			if ($subws==0){
				$subws=strlen($NoRule[1]);//编码长度
			}
			if (strlen($_POST['StockID'])!=$codelen){
				$InputError = 1;
				prnMsg ('系统已经设置物料编码长度 '.$codelen.'位,当前编码 '.strlen($_POST['StockID']),'error');
				$Errors[$i] = 'StockIDLen';
				$i++;
			}
		}
	...
	if ($_SESSION['StockMode']==1){//物料添加简易处理
		$_POST['Volume']=0;
	}else{
		...
		if ($New==0) { /*so its an existing one */
			...
			*/
			...
			if ($InputError == 0){
				...
				$sql = "UPDATE stockmaster
						SET longdescription='" . $_POST['LongDescription'] . "',
							...
							categoryid=getcategory('".$StockID."'),
							units='" . $_POST['Units'] . "',
							mbflag='" . $_POST['MBFlag'] . "',
				...
				/*First get the total cost of WIP for this category */
					...
					$WOCostsResult = DB_query("SELECT workorders.costissued,
												...
												ON woitems.wono = workorders.wono
			...
			if (DB_num_rows($result)==1){
				...
				//不是自动生成stockid缺少	'" . $_POST['CategoryID'] . "',
				DB_Txn_Begin();
...
if (!isset($StockID) OR $StockID=='' or isset($_POST['UpdateCategories'])) {
	...
	if ($New==1) {
		...
		$sql = "SELECT subcategoryid,categoryid, subcategorydspn categorydescription 
					FROM stocksubcategory 
					WHERE categoryid in ( SELECT loccode FROM locationusers WHERE userid='".$_SESSION['UserID']."')";
		$ErrMsg = _('The stock categories could not be retrieved because');
		$DbgMsg = _('The SQL used to retrieve stock categories and failed was');
		/*if (isset($_SESSION['AutoCode']) && $_SESSION['AutoCode']==1 ){
			echo '<tr>
			<td>1008' . _('Category') . ':</td>
			<td><select name="SubCategory" onchange="ReloadForm(ItemForm.UpdateCategories)">';
		
			$result = DB_query($sql,$ErrMsg,$DbgMsg);

			while ($myrow=DB_fetch_array($result)){
				if (isset($_POST['SubCategory']) &&(  $myrow['categoryid'].'^'. $myrow['subcategoryid']==$_POST['SubCategory'])){
					echo '<option selected="selected" value="'.  $myrow['categoryid'].'^'.$myrow['subcategoryid'] . '">'  .$myrow['subcategoryid'].'-'. $myrow['categorydescription'] . '</option>';
				} else {
					echo '<option value="'. $myrow['categoryid'].'^'.  $myrow['subcategoryid'] . '">' .$myrow['subcategoryid'].'-'. $myrow['categorydescription'] . '</option>';
				}
				
			}
			
		}else*/if(isset($_SESSION['AutoCode'])){
			echo '<tr>
			<td>' . _('Category') . ':</td>
			<td><select name="SubCategory" onchange="ReloadForm(ItemForm.UpdateCategories)">';
			...
			while ($myrow=DB_fetch_array($result)){
				if (isset($_POST['SubCategory']) &&(  $myrow['categoryid'].'^'. $myrow['subcategoryid']==$_POST['SubCategory'])){
					echo '<option selected="selected" value="'.  $myrow['categoryid'].'^'.$myrow['subcategoryid'] . '">' . $myrow['categorydescription'] . '</option>';
				} else {
					echo '<option value="'. $myrow['categoryid'].'^'.  $myrow['subcategoryid'] . '">' .$myrow['subcategoryid'].'-'. $myrow['categorydescription'] . '</option>';
				}
			...
			echo '</select>
			          <input type="text" name="subStockid"  maxLength=12 size=15   pattern="^\w{3,7}$"  placeholder="空白,系统自动编号" title="格式：数字字母，3位以上小于8位！"></td>
			</tr>';
...
//$sql="SELECT description  taxcatname, taxid taxcatid, taxrate FROM taxauthorities WHERE onorder IN(1,2,3 )";
$sql = "SELECT taxcatid, taxcatname FROM taxcategories WHERE onorder =2 OR onorder=3 ORDER BY taxcatname";
$result = DB_query($sql);
...
// prnMsg($NEW .'='.$_POST['SubCategory'],'info');	
echo '</div>
		</div>
	</form>';
include('includes/footer.php');
function CreateCode($SubCategory,$norule){		
	//自动产生科目好，顺序产生有漏号补齐
	$n=(int)$norule[0];
	$dbtno=(int)$norule[1];
	$subws=strlen($norule[1]);//编码长度
	$ws=strlen($SubCategory);//类长度
	if (isset($norule[2])){
		$codelen=$norule[2];
	}else{
		$codelen=$ws+$subws;
	}
	$SQL="SELECT stockid FROM `stockmaster` WHERE stockid LIKE '".$SubCategory."%' AND length(stockid)=".($codelen)."  ORDER BY stockid ";
	//prnMsg($SQL);
	$Result=DB_query($SQL);
	$subarr=array();	
	$stockid='';
	$i=1;
	if (DB_num_rows($Result)==0){//子类下为空
		//prnMsg('1605子类下为空');
		$stockid=$SubCategory.$norule[0];
	}else{
		
		while ($row=DB_fetch_array($Result)){
		
			if (mb_strlen($row['stockid'])==$codelen){
			
				if ($SubCategory.str_pad((string)$i ,($codelen-$ws),"0",STR_PAD_LEFT)==$row['stockid']){
		...
		if ($stockid==''){
			...
			$stockid=$SubCategory.str_pad((string)$i ,$subws,"0",STR_PAD_LEFT);
		}
