...
If (isset($_POST['PrintPDF'])||isset($_POST['CSV'])||isset($_POST['Search'])) {
...
If (isset($_POST['PrintPDF'])) {
	...
	$CSVListing =  iconv("UTF-8","gbk//TRANSLIT",_('Stock ID') ).','.iconv("UTF-8","gbk//TRANSLIT", _('Description')) .','.iconv("UTF-8","gbk//TRANSLIT", '启用日期') .','. iconv("UTF-8","gbk//TRANSLIT",'到期日期' ).','.iconv("UTF-8","gbk//TRANSLIT", _('Units'))  .','.iconv("UTF-8","gbk//TRANSLIT", '价格'). "\n";
	while ($InventoryValn = DB_fetch_array($PricesResult)) {
		$CSVListing .= '"';
		
		$CSVListing .=iconv("UTF-8","gbk//TRANSLIT",'`'.$InventoryValn['stockid'].'","'.$InventoryValn['description'].'","'.$InventoryValn['startdate'].'","'.$InventoryValn['enddate'].'","'.$InventoryValn['units'].'","'.$InventoryValn['price']).'"'. "\n";
		//$CSVListing .=iconv("UTF-8","gbk//TRANSLIT",implode('","', $csvarr) ). '"' . "\n";itemtotal
	}
	header('Content-Encoding: UTF-8');
    header('Content-type: text/csv; charset=UTF-8');
    header("Content-disposition: attachment; filename=".iconv("UTF-8","gbk//TRANSLIT",'价格表_').  date('Y-m-d')  .'.csv');
...
if (isset($_POST['Search']) OR isset($_POST['Go']) OR isset($_POST['Next']) OR isset($_POST['Previous'])) {
	...
	$ListCount = DB_num_rows($PricesResult);
	...
	$RowIndex = 0;
	//if (DB_num_rows($PricesResult) <> 0) {
		DB_data_seek($PricesResult, ($_POST['PageOffset'] - 1) * $_SESSION['DisplayRecordsMax']);
	//}
	while (($myrow = DB_fetch_array($PricesResult)) AND ($RowIndex <> $_SESSION['DisplayRecordsMax'])) {
		if ($k == 1) {
