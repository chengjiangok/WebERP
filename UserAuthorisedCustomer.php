...
<?php
/* $Id: UserAuthorisedCustomer.php  $*/
/*
 * @Author: ChengJiang 
 * @Date: 2018-01-01 16:43:32 
 * @Last Modified by: ChengJiang
 * @Last Modified time: 2018-11-20 16:23:28
 */

include('includes/session.php');
$Title = '用户授权客户';
$ViewTopic = 'UserAuthorisedCustomer';
$BookMark = 'UserAuthorisedCustomer';
...
if(!isset($SelectedUser)) {// If is NOT set a user for GL accounts.
	...
	if(isset($_POST['submit'])) {
		if(!isset($SelectedCustCode)) {
			...
			if($CheckRow[0] > 0) {
				...
				if(DB_query($SQL, $ErrMsg)) {
					prnMsg( $SelectedUser . ' 对选择的客户[编码: ' . $SelectedCustCode . ']授权',  'success');
					unset($_GET['SelectedCustCode']);
					unset($_POST['SelectedCustCode']);
				}
			}
		...
		$SQL = "DELETE FROM customerusers
					...
					AND (ustype=1 OR custype=3)
					AND userid='" . $SelectedUser . "'";
			$ErrMsg ='对选择的客户授权撤销没有完成';
			if(DB_query($SQL, $ErrMsg)) {
			prnMsg( $SelectedUser . '对选择的客户[编码: ' . $SelectedCustCode . ']授权撤销', 'success');
			unset($_GET['delete']);
			unset($_POST['delete']);
		}
		...
		$SQL = "UPDATE customerusers
				...
				AND userid='" . $SelectedUser . "'";
		$ErrMsg = '对选择的客户授权修改没有完成';
		if(DB_query($SQL, $ErrMsg)) {
			prnMsg($SelectedUser . ' 对选择的客户[编码:  ' . $SelectedCustCode . ']跟新权撤销', 'success');
	...
	echo '<table class="selection">
	 ...
	 $Result=DB_query("SELECT regid custcode, name custname, custype, canview, canupd 
						FROM customerusers 
						left JOIN debtorsmaster
					ON debtorsmaster.debtorno=customerusers.regid
						WHERE customerusers.userid='" . $SelectedUser . "'
						AND  (customerusers.custype=1 OR customerusers.custype=3)
	...
	echo '</tbody></table>',
		...
		<table class="selection noprint">
			<tr>
				<td>';
	//$Result = DB_query("SELECT concat(`supplierid`,'^') custcode,  1 custype,`suppname` custname FROM `suppliers` UNION SELECT regid custcode,2 custype, brname custname FROM debtorsmaster 
	
	$Result = DB_query("SELECT debtorno custcode, name custname 
					FROM debtorsmaster
					WHERE NOT EXISTS (SELECT   canupd  FROM customerusers   WHERE  debtorsmaster.debtorno=regid
	 	  			AND  customerusers.userid='" . $SelectedUser . "')");
	...
	echo '<br>', // Form buttons:
		'<div class="centre noprint">',
			...
			'<button formaction="UserAuthorisedCustomer.php?Cancel" type="submit"><img alt="" src="', $RootPath, '/css/', $Theme,
				'/images/user.png" /> ', _('Select A Different User'), '</button>'; // "Select A Different User" button.
}
echo		'<button formaction="UserAuthorisedCustomer.php" type="submit"><img alt="" src="', $RootPath, '/css/', $Theme,
