<?php
/* $Id: GLSettleSales.php$*/
/* SettleSales for employees  */
...
echo '<p class="page_title_text"><img src="'.$RootPath.'/css/'.$Theme.'/images/magnifier.png" title="' . _('Search') . '" alt="" />' . ' ' . $Title . '</p>
	...
	<br />';
		...
		$SQL="SELECT  periodno,
					...
					WHERE	(account LIKE '6001%' OR account LIKE '6401%' )					 
...
if (DB_num_rows($Result)>0){
		echo '<table class="selection">';
		echo'<tr>
		<th colspan="6"   class="centre">'.substr($_POST['SelectDate'],0,4).'年营业收入成本</th>';
	...
	$SQL=" SELECT   gltrans.tag,
					...
					WHERE periodno <=".$SelectPrd."
						...
						AND (account LIKE '6001%' OR account LIKE '6401%' )
							group by  gltrans.tag, account,accountname";
	$Result = DB_query($SQL);			
	if (DB_num_rows($Result)>0){
		echo'
			<tr>
				<th colspan="6"><h3>结账期营业收入成本发生额</h3></th>
			<tr>
			<th >月份</th>
			<th >分组</th>				
			<th >科目编码/名称</th>
		...
		while ($row = DB_fetch_array($Result)) {
			...
			$AmoTotal+=$row['qmye'];
			if ($row['tag']>0){
				$ToActAmo[$row['account']][0]=$row['qmye'];
				$ToActAmo[$row['account']][1]=$row['occur_de'];
				$ToActAmo[$row['account']][2]=$row['occur_cr'];
			}
	...
	echo '</table>
		...
		//$SettleTabAct=array("0"=>'1602',"1"=>'2211',"2"=>'2221',"3"=>"1403","4"=>"1407","7"=>'7001',"6"=>'7101',"7"=>'6001');
	    $SettleTab=explode(',',$_SESSION['CompanyRecord'][$_POST['UnitsTag']]['settle']);		
		if ($SettleTab[7]>=1){
		
			prnMsg(substr($_POST['SelectDate'],0,7).'销售成本已经结转!','info');
			include('includes/footer.php');
			exit;	
		}
	
if(isset($_POST['SettleSales'])||isset($_POST['Updateing'])) {
	   /*1.只能结转外账
		*/
	   ...
	   echo'<tr>
			...
			foreach($SettleRule as  $key=>$row){
				...
				if (strlen($CostActStr)>5){
					...
					echo'<td rowspan="'.$rw2.'">'.$RowIndex.'-'.$rw2.'</td>			      
							<td rowspan="'.$rw2.'">['.$key.']'.$row[0].'</td>
							<td rowspan="'.$rw2.'"  class="number">
							
							<input type="hidden"  name="Rmk'.$key.'" id="Rmk'.$key.'"   value="月末结转销售成本"  />
							...
							foreach($ToActArr as $keyAct=> $val){
								...
								if(isset($ToActAmo[$keyAct])){
									$ToAmo=round($ToActAmo[$keyAct][2]*$val,2);
								}
							}
					*/
						$ToAmoTotal=0;
						if ($rw2>0){
							foreach($ToActArr as $keyAct=> $val){
								...
								$ToAmo=round($ToActAmo[array_keys($val[0])[0]][2]*$val[0][array_keys($val[0])[0]],2);
			
								if ($_POST[$key.'Edit'.$keyAct]==2){
									$SettleRule[$key][6]= $_POST[$key.'ToAmo'.$keyAct];
									$ToAmoTotal+=$_POST[$key.'ToAmo'.$keyAct];
								}else{
									$SettleRule[$key][6]=$ToAmo;;
									$ToAmoTotal+=$ToAmo;;
								}
								$_POST[$key.'ToAmo'.$keyAct]=$ToAmo;
								//$_POST[$key.'Remark'.$keyAct]=$val[5];	
							
								//$_POST[$key.'Remark'.$rw]=$val[5];				
								echo $tr;					
								echo'<td >['.$keyAct.']'.$ToActName[$keyAct].'
									<input type="hidden"  name="'.$key.'Edit'.$keyAct.'" id="'.$key.'Edit'.$keyAct.'"   value="1" />
									<input type="hidden"  name="'.$key.'ToAct'.$keyAct.'" id="'.$key.'ToAct'.$keyAct.'"   value="' .$keyAct. '" /></td>				
								<td  class="number"> 
									<input type="text"  class="number" name="'.$key.'ToAmo'.$keyAct.'" id="'.$key.'ToAmo'.$keyAct.'"  size="10"   pattern="(^-?\d{1,10})(.\d{1,2})?$"　  value="'. $_POST[$key.'ToAmo'.$keyAct].'"  onchange="OnToAmo(this ,'.$rw2.','.$keyAct.')" />	</td>
								<td > <input type="text"  class="number" name="'.$key.'Remark'.$keyAct.'"   id="'.$key.'Remark'.$keyAct.'"  size="15"  value="'.$_POST[$key.'Remark'.$keyAct].'"   title="'.$msg.'"  pattern="[\w\d\u0391-\uFFE5\(\)\[\]\ +$"  placeholder="月末结转销售成本'.substr(strstr($val[1],'-'),1).'" onchange="OnRemark(this ,'.$key.')"  />	</td>';
								echo'</tr>';
								if($rw2>1){
								$tr='<tr>';
								}
		...
		if(isset($_POST['Updateing'])){
			...
			foreach ($_POST as $FormVariableName =>$Qty) {
				...
				if (round($AmoTotal,2)==0){
						...
						$SettleTab[7]=$SettleTab[7]+1;
						if ($InitialTab[7]==0){
							$SettleTab[7]=$SettleTab[7]+1;
						}elseif ($InitialTab[7]==-1){
							$SettleTab[7]=$SettleTab[7]+2;
						}
						$Status=0;
					...
					prnMsg('销售成本结转会计凭证: ' . $TransNo . ' ' . _('has been successfully entered'),'success');
...
echo '<div class="centre">
			<input type="submit" name="SettleSales" value="结转销售成本" />	';
	if(isset($_POST['SettleSales'])) {
