...
<?php
/* $Id: FixedAssetCategories.php 6941  2017-01-18 10:40:34  ChengJiang daintree $*/
/*
 * @Author: ChengJiang 
 * @Date: 2017-07-05 13:09:03 
 * @Last Modified by: ChengJiang
 * @Last Modified time: 2017-07-05 13:09:24
 */
...
//SQL to poulate account selection boxes    accountgroups.pandl=0
$sql = "SELECT accountcode,
				 accountname
		FROM chartmaster INNER JOIN accountgroups
		ON chartmaster.group_=accountgroups.groupname
		WHERE (chartmaster.accountcode like '1701%' or chartmaster.accountcode like '1601%' ) and CHAR_LENGTH(chartmaster.accountcode )>4
...
$sql = "SELECT  accountcode,
			...
			ON    chartmaster.group_ = accountgroups.groupname
			WHERE (chartmaster.accountcode LIKE '1701%' OR chartmaster.accountcode LIKE '1601%' OR chartmaster.accountcode LIKE '5%' OR chartmaster.accountcode LIKE '66%'  ) 
			AND CHAR_LENGTH(chartmaster.accountcode) > 4 AND accountname LIKE '%折旧%'
			ORDER BY accountcode";
...
$sql = "SELECT accountcode,
		...
		FROM chartmaster INNER JOIN accountgroups
		ON chartmaster.group_=accountgroups.groupname
		WHERE (chartmaster.accountcode like '1702%' or chartmaster.accountcode like '1602%') and CHAR_LENGTH(chartmaster.accountcode )>4 

		ORDER BY accountcode";

$DpAccountsResult = DB_query($sql);
/*
$sql = "SELECT accountcode,
				 accountname
		FROM chartmaster INNER JOIN accountgroups
		ON chartmaster.group_=accountgroups.groupname
		WHERE chartmaster.accountcode like '1606%' and CHAR_LENGTH(chartmaster.accountcode )>4 

		ORDER BY accountcode";

$PLAccountsResult = DB_query($sql);
...
while ($myrow = DB_fetch_array($PLAccountsResult)) {
...
while ($myrow = DB_fetch_array($DpAccountsResult)) {
