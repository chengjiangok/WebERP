...
$FieldHeadings = array(
	'AccountCode',			//  0 'Account Code
	'Description',		//  1 'Account Description',
	'AccountGroup',		//  2 'Account Group',
	'Tag'
);
...
if (isset($_FILES['ChartFile']) and $_FILES['ChartFile']['name']) { //start file processing
	...
	/*	$sql = "SELECT COUNT(group_) FROM chartmaster WHERE group_='" . iconv("gb2312","UTF-8", $myrow[2]) . "'";
		$result = DB_query($sql);
		$testrow = DB_fetch_row($result);
		if ($testrow[0] == 0) {
			$InputError = 1;
			prnMsg (_('Account Group') . ' "' .iconv("gb2312","UTF-8", $myrow[2]). '" ' . $sql._('does not exist. First enter the account groups you require in webERP before attempting to import the accounts.'),'error');
		}
 */
		if ($InputError !=1){
			...
			$sql = "INSERT INTO chartmaster (accountcode,
										...
										'".trim($myrow[0])."',
										'".trim(iconv("gb2312","UTF-8", $myrow[1]))."',
										'".trim(iconv("gb2312","UTF-8", $myrow[2]))."'
											,'".trim($myrow[3])."'
											,'".date('Y-m-d')."')";
	...
	echo '<div class="page_help_text">
			������Ŀ������<br />' 
		.	'AccountCode,Description,AccountGroup,Tag,crtdate<br />'.
		'The Account Group field must have a corresponding entry in the account groups table. So these need to be set up first.' . '</div>';
