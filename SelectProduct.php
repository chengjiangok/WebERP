<?php
/* $Id: SelectProduct.php 7494 2017/1/26 6:21:42Zchengjiang daintree $*/
...
if (isset($_GET['Search']) or isset($_POST['Next']) or isset($_POST['Previous']) or isset($_POST['Go'])) {
	unset($StockID);
...
$SQL = "SELECT categoryid,  categorydescription
				...
				WHERE stocktype = 'B' or stocktype = 'M'
...
if (!isset($_POST['CSV'])) {
	...
	if (!isset($_SESSION['SelectedStockItem'] )) {
		echo'<input type="submit" name="CSV" value="导出CSV" /></div><br />';
	}
	...
	if (isset($_POST['Keywords']) AND mb_strlen($_POST['Keywords'])>0) {
		...
		if ($_POST['StockCat'] == 'All') {
			$SQL = "SELECT stockmaster.stockid,
						...
						AND CONCAT(stockmaster.description,stockmaster.longdescription) " . LIKE . " '$SearchString'
			...
			$SQL = "SELECT stockmaster.stockid,
						...
						WHERE CONCAT(stockmaster.description,stockmaster.longdescription) " . LIKE . " '$SearchString'
...
if (isset($_POST['CSV'])) {
	...
	$CSVListing ='"';
	$CSVListing .=iconv("UTF-8","gbk//TRANSLIT",'产品编码').'","'.iconv("UTF-8","gbk//TRANSLIT",'客户产品编码').'","'.iconv( "UTF-8", "gb2312","产品名称").'","'.iconv( "UTF-8", "gb2312","产品描述").'","'.iconv( "UTF-8", "gb2312","类别").'","'.iconv( "UTF-8", "gb2312","标记" ).'","'.iconv( "UTF-8", "gb2312","数量" ).'","'.iconv( "UTF-8", "gb2312","单位" ).'","'.iconv( "UTF-8", "gb2312","保留小数" ).'"'. "\n";
	...
	header('Content-disposition: attachment; filename='.iconv( "UTF-8", "gb2312","物料列表_") .  date("Y-m-d")  .'.csv');
...
if (isset($SearchResult) AND !isset($_POST['Select'])&& !isset($_POST['CSV'])) {
	echo '<form action="' . htmlspecialchars($_SERVER['PHP_SELF'],ENT_QUOTES,'UTF-8') . '" method="post">';
    echo '<div>';
	echo '<input type="hidden" name="FormID" value="' . $_SESSION['FormID'] . '" />';
	...
	if ($ListCount > 0) {
		...
		$TableHeader = '<tr>
							...
							<th class="ascending">' . _('Long Description') . '</th>
