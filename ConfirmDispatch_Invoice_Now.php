...
if (!isset($_GET['OrderNumber']) AND !isset($_SESSION['ProcessingOrder'])) {
	...
	$OrderHeaderSQL = "SELECT salesorders.orderno,
						...
						FROM salesorders 
						LEFT JOIN  registername
						ON salesorders.debtorno = registername.regid					
		...
		if (DB_num_rows($LineItemsResult)>0) {
			...
			while ($myrow=DB_fetch_array($LineItemsResult)) {
				...
				$_SESSION['Items'.$identifier]->add_to_cart($myrow['stkcode'],
															...
															$myrow['taxcatid'],
															$myrow['standardcost'],
															htmlspecialchars_decode($myrow['narrative'])//27
...
echo '<table class="selection">
			...
			<tr>
				<td class="label">合同号:</td><td>'.$_SESSION['Items'.$identifier]->OrderNo.'</td><td class="label"><b>' . _('Invoice amounts stated in') . '</b></td><td><b> ' . $_SESSION['Items'.$identifier]->DefaultCurrency . '</b></td>
			</tr>
			<tr>
			<td class="label">客户合同号:</td><td>'.$_SESSION['Items'.$identifier]->CustRef.'</td><td></td><td></td>
		</tr>
	</table><br />';
...
echo '<table width="90%" cellpadding="2" class="selection">
	<tr>
		...
		echo'<th>订单价目<br/> ['.$_SESSION['CountryOfOperation'].  ']</th>
			<th>税目</th>
			<th>税额</th>
...
foreach ($_SESSION['Items'.$identifier]->LineItems as $LnItm) {
	...
	if ( $_SESSION['Items'.$identifier]->CurrCode!=CURR){
        $col=4;
		echo '<td class="number">'.locale_number_format( $LnItm->CurrPrice ,$LnItm->DecimalPlaces).
	...
	echo '<td class="number">' .locale_number_format( $LnItm->TaxPrice,$_SESSION['Items'.$identifier]->CurrDecimalPlaces) . '
		  ...
		  <td class="number">' .  $_SESSION['Items'.$identifier]->TaxCatID . '
		  <input  type="hidden"  name="' . $LnItm->LineNumber . '_TaxRate" id="' . $LnItm->LineNumber . '_TaxRate"   value="' .$_SESSION['Items'.$identifier]->TaxRate. '" /></td>';
		//合计
		echo '<td class="number"><input type="text"  class="number"  name="' . $LnItm->LineNumber . '_TaxTotal"  id="' . $LnItm->LineNumber . '_TaxTotal"  maxlength="5" size="5"  value="' . locale_number_format($LineTaxTotal ,$LnItm->DecimalPlaces) . '" readonly="readonly"  /></td>';
		echo '<td class="number"><input  type="text"  class="number"  name="' . $LnItm->LineNumber . '_LineTotal" id="' . $LnItm->LineNumber . '_LineTotal" maxlength="8" size="8"   value="' . locale_number_format($LineTotal,$LnItm->DecimalPlaces) . '" readonly="readonly"  /></td>';
	if ( $_SESSION['Items'.$identifier]->CurrCode!=CURR){
		$LineCurrTotal=$LnItm->QtyDispatched * $LnItm->CurrPrice; 
		echo '<td class="number"><input  type="text"  id="' . $LnItm->LineNumber . '_CurrTotal"  maxlength="8" size="8"    value="' . locale_number_format($LineCurrTotal ,$LnItm->DecimalPlaces) . '"   readonly="readonly" />';
		
	}else{
		echo'<input  type="hidden"  id="' . $LnItm->LineNumber . '_CurrTotal"    value="' . locale_number_format($CurrTotal,$LnItm->DecimalPlaces) . '"   />';
	}
	$AmoTotal+=$LineTotal;	
...
echo '<td class="number"><input  type="text"  class="number"  id="TaxTotal" maxlength="8" size="8"   value="' . locale_number_format($TaxTotal,$LnItm->DecimalPlaces) . '" readonly="readonly"  /></td>
	  <td class="number"><input  type="text"  class="number"  id="AmoTotal" maxlength="8" size="8"   value="' . locale_number_format($AmoTotal,$LnItm->DecimalPlaces) . '" readonly="readonly"  /></td>';
if ( $_SESSION['Items'.$identifier]->CurrCode!=CURR){
	echo'<td class="number"><input  type="text"   id="CurrTotal" maxlength="8" size="8"   value="' . locale_number_format($CurrTotal,$LnItm->DecimalPlaces) . '" readonly="readonly"  /></td>';
}else{
	echo'<input  type="hidden"  id="CurrTotal"    value="' . locale_number_format($CurrTotal,$LnItm->DecimalPlaces) . '"   />';
}
	echo'</tr>';
...
if (isset($_POST['ProcessInvoice']) && $InputErr==false AND $_POST['ProcessInvoice'] != ''){
	...
	/*Now need to check that the order details are the same as they were when they were read into the Items array. If they've changed then someone else may have invoiced them */
		...
		/*there should be the same number of items returned from this query as there are lines on the invoice - if  not 	then someone has already invoiced or credited some lines */
			...
			echo '<br />';
			prnMsg('660'. _('This order has been changed or invoiced since this delivery was started to be confirmed') . '. ' . _('Processing halted') . '. ' . _('To enter and confirm this dispatch') . '/' . _('invoice the order must be re-selected and re-read again to update the changes made by the other user'), 'error');
		...
		while ($myrow = DB_fetch_array($Result)) {
			...
			if ($_SESSION['Items'.$identifier]->LineItems[$myrow['orderlineno']]->Quantity != $myrow['quantity']
				...
				prnMsg('677'. _('This order has been changed or invoiced since this delivery was started to be confirmed') . ' ' . _('Processing halted.') . ' ' . _('To enter and confirm this dispatch, it must be re-selected and re-read again to update the changes made by the other user'), 'error');
