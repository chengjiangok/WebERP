...
echo '<form action="' . htmlspecialchars($_SERVER['PHP_SELF'],ENT_QUOTES,'UTF-8') . '" method="post">
	...
	echo ''. _('Payroll Group') . ':</td>
	     <td>
		   <select  id="select-payrollgroup" name="PayrollGroupID"> 
		   <option value="">select payroll group</option>';
    $sql = "SELECT payrollgroup_id, payrollgroup_name,frequency_name
	             FROM hrpayrollgroups 
				 JOIN hrpaymentfrequency ON hrpayrollgroups.payment_frequency=hrpaymentfrequency.paymentfrequency_id";
    ...
    while ($myrow=DB_fetch_array($resultPayrollGroups)){
	  ...
	  echo '</select></td>
	 ';
	  /* &nbsp;&nbsp'._('Payment Period') . ':
	         <input type="text" name="ERPPrd" required="required"  id="datepicker" maxlength="10" size="10"  />&nbsp';
	 */
...
//	if (isset($_POST['Search'])|| $_POST['PageOffset']>1||isset($_POST['Go']) ||isset($_POST['Next']) ||isset($_POST['Previous'])){			
	echo '<input type="submit" name="GeneratePayroll" value="' . _('Generate ') . '"/> ';
	//onclick="return confirm(\'', _('Are you sure you wish to generate payroll?'), '\');" />';
//	}
echo '</div>';
        echo'<br />
        </div>
...
if(isset($GeneratedPayroll))
{
	...
	echo '<table>
					...
					<td>'.$PayslipEmployees.' of '.$EmployeesInPaygroup.' Approved: '.$ApprovedPayslips.' Pending: '.$PendingPayslips.' </td>
					</tr>
			</table>';
}
echo '<script>
...
if(isset($_POST['GeneratePayroll'])) {
	//prnMsg($_POST['GeneratePayroll'].'GeneratePayroll-Debug-94=PayrollGroupID=='.$_POST['PayrollGroupID']);
			$InputError = 0;
			if (trim($_POST['PayrollGroupID']) == '') {
				$InputError = 1;
				prnMsg(_('please select Payroll group'),'error');//请选择工资组
				$Errors[$i] = 'PayrollGroupID';
				$i++;
			}
			if($InputError == 0)
			{
			$GeneratedPayroll = $_POST['GeneratePayroll'];//执行按钮

			$PayrollGroupID = $_POST['PayrollGroupID'];//群组ID
			...
			$sql = "SELECT hrpayrollgroups.*, frequency_name 
			             FROM hrpayrollgroups
				         JOIN hrpaymentfrequency ON hrpayrollgroups.payment_frequency=hrpaymentfrequency.paymentfrequency_id 
			             WHERE payrollgroup_id ='".$PayrollGroupID."'";
			$result = DB_query($sql);
			...
			if (DB_num_rows($checksql) > 0)
			{
					...
					$sql = "SELECT hremployeepayslips.*, first_name,middle_name,last_name,employee_department 
					           FROM hremployeepayslips
							    JOIN hremployees on hremployeepayslips.employee_id = hremployees.empid
					...
					while($myrow = DB_fetch_array($result))
					{
						...
						$sql2 = "SELECT hremployeesalarystructure_components.* ,payroll_category_code,payroll_category_type 
						                from hremployeesalarystructure_components 
									    JOIN hrpayrollcategories ON hremployeesalarystructure_components.payroll_category_id = hrpayrollcategories.payroll_category_id
						...
						//get other earnings.
						$sql2 = DB_query("SELECT sum(amount) as other_earnings 
						                            FROM hrpayslipextradetails 
													WHERE entry_type = 1 AND payslip_id='".$payslip_id."'");
						$earning_result = DB_fetch_array($sql2);
						$other_earnings = $earning_result['other_earnings'];

						//get other deductions
						$sql2 = DB_query("SELECT sum(amount) as other_deductions 
						                  FROM hrpayslipextradetails 
										  WHERE entry_type = 0 AND payslip_id='".$payslip_id."'");
				...
				//start transaction
				//$result = DB_Txn_Begin();
				$sql = "INSERT INTO hrpayslipdateranges(start_date,
				                                         end_date,
														 payrollgroup_id)
															VALUES(
																'" . $StartDate. "',
				...
				$sql ="SELECT hremployees.*,salary_structure_id,gross_pay,net_pay
						...
						WHERE hremployees.status = 1 
						    AND hremployeesalarystructures.payrollgroup_id ='" . $PayrollGroupID . "'";
				$result = DB_query($sql);
				$payroll_array = array();
				$i=0;
				$PendingPayslips = 0;
			...
			//if (DB_error_no() ==0) {
				...
				echo '<table>
							<tr>
							   <td>Pay period</td>
							   <td>Payment frequency</td>
							   <td>Payslips generated</td><td>Total Net Salary</td>
							   <td>Bulk actions</td>
							   </tr>
								<tr>
								<td><strong>'.$_POST['ERPPrd'].'</strong></td>
								<td><strong>'.$PayrollGroupFrequency.'</strong></td>
								<td>'.$PayslipEmployees.' of '.$PayrollGroupEmployees.' |  Pending: '.$PendingPayslips.' Approved: '.$ApprovedPayslips. ' Paid : '.$PaidPayslips.'</td>
								<td><strong>'.$payroll_currency.' '.number_format($TotalNetPay,2).'</strong></td>
								<td><form id="bulkActionsForm" method="post">
								     <input type="hidden" name="FormID" value="' . $_SESSION['FormID'] . '" />
...
*/
		echo '<table class="selection">
				<tr>
					...
					$sql= "SELECT payroll_category_name,payroll_category_type,payroll_category_code,payroll_category_value,payroll_category_type,additional_condition
								 FROM hrpayroll_groups_payroll_categories
								  JOIN hrpayrollgroups ON hrpayroll_groups_payroll_categories.payroll_group_id = hrpayrollgroups.payrollgroup_id
				...
				while($row = DB_fetch_array( $Result)){
					...
					echo'	<td>'.number_format($row['employee_other_earnings'],2).'</td>
								<td>'.number_format($row['employee_total_earnings'],2).'</td>';
								foreach($row['employee_deductions'] as $deductions)
								{
										echo '<td>'.number_format($deductions['amount'],2).'</td>';
								}
					echo'	<td>'.number_format($row['employee_other_deductions'],2).'</td>
								<td>'.number_format($row['employee_total_deductions'],2).'</td>
								<td>'.number_format($row['employee_net_pay'],2).'</td>';
						//	echo	'<td class="noprint"><a href="HrEmployees.php?EmpID='. $row['employee_id']. '">'. _('Edit'). '</a></td>';
								if($row['payslip_status'] == "pending")
								{
								...
								else if($row['payslip_status'] == "paid")
								{
		    					echo '<td class="noprint"><a href="HrPrintPayslip.php?PayslipNo='. $row['payslip_id']. '">'. _('Print Payslip'). '</a></td>';
								}
					echo '</tr>';
				}
		//	}
	...
	if(isset($_POST['BulkOption']))
	{
		...
		else if($_POST['BulkOption'] == "pay_all")
		{
			...
			if(!in_array('10',$_SESSION['AllowedPageSecurityTokens']))
			{
				...
				$sql = DB_query("SELECT bank_account_to_use,gl_posting_account,currency 
				                     FROM hrpayrollgroups 
									 WHERE payrollgroup_id = '".$PayrollGroupID."'");
				$result = DB_fetch_array($sql);
