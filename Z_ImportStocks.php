 <?php
/* $Id: Z_ImportStocks.php 7347 2017/1/26 15:20:41Z chengjiang daintree $*/
...
if (isset($_FILES['userfile']) AND $_FILES['userfile']['name']) { //start file processing
	...
	//get the header row
	$headRow = fgetcsv($FileHandle,",");  // Modified to handle " "" " enclosed csv - useful if you need to include commas in your text descriptions

	//check for correct number of fields
//prnMsg(count($headRow).'--'. count($FieldHeadings),'info');
	...
	foreach ($headRow as $headField) {
		if ( mb_strtoupper(trim($headField)) != mb_strtoupper(trim($FieldHeadings[$head])) ) {
			prnMsg(mb_strtoupper($headField).' !='. mb_strtoupper($FieldHeadings[$head]),'info');
	...
	while ( ($myrow = fgetcsv($FileHandle, 10000, ",")) !== FALSE ) {
		...
		if ($InputError !=1){
	        ...
	        lowestlevel)
				VALUES (
					'".trim($myrow[0])."',
					'" . iconv('GB2312', 'UTF-8', $myrow[1])	. "',
					'" . iconv('GB2312', 'UTF-8', $myrow[2])	. "',
					'" . $myrow[3]	. "',
					'" . iconv('GB2312', 'UTF-8', $myrow[4])	. "',
					'"  . $myrow[5]	. "',
		  		'"  . $myrow[6]	. "',
					'"  . $myrow[7]	. "',
					'"  . $myrow[8]	. "',
			...
			/* $sql = "INSERT INTO locstock (loccode,
									...
									'" . $StockID . "', (case locations.loccode when '".$myrow[21]."' then  '".$myrow[20]."'  else  0 end ) quantity
									FROM locations";*/
