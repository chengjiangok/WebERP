...
if (isset($_POST['submit'])) {
	...
	if (mb_strlen($_POST['Code']) !=1) {
		$InputError = 1;
		prnMsg(_('The Work Centre code must be at least 1 characters long'),'error');
	}
...
or deletion of the records*/
	...
	echo '<table class="selection">
			<tr>
				...
				<th class="ascending">','制造费用分摊系数', '</th>
				<th class="ascending">人工费总账科目</th>
				<th class="ascending">人工费分摊系数</th>
				<th colspan="2">&nbsp;</th>
			</tr>';
...
if (isset($SelectedWC)) {
	...
	$_POST['LabourPerHr'] = $myrow['labourperhr'];
	...
	echo '<table class="selection">
			<tr>
				<td>' . _('Work Centre Code') . ':</td>
				<td><input type="text" name="Code" pattern="[^&+-]{1,}" required="required" autofocus="autofocus" title="'._('The code should be at least 1 characters and no illegal characters allowed').'"  size="6" maxlength="5" value="' . $_POST['Code'] . '" placeholder="'._('More than 1 legal characters').'" /></td>
			</tr>';
}
...
echo '<tr>
		<td>人工费总账科目:</td>
		<td><input type="text" class="number" name="LabourAct" size="6" title="'._('The input must be numeric').'" maxlength="20" value="'.$_POST['LabourAct'].'" />';
echo '</td>
      <tr>
		<td>人工费分摊系数:</td>
		<td><input type="text" class="number" name="LabourPerHr" size="6" title="'._('The input must be numeric').'" maxlength="6" value="'.$_POST['LabourPerHr'].'" />';
