...
/* $Id: BOMs.php  ChengJiang $*/
/*
 * @Author: ChengJiang 
 * @Date: 2018-10-03 15:05:37 
 * @Last Modified by: ChengJiang
 * @Last Modified time: 2018-10-03 15:06:15
 */
...
function DisplayBOMItems($UltimateParent, $Parent, $Component,$Level, $db) {
  //读取BOM  资料
		...
		$result = DB_query($sql,$ErrMsg,$DbgMsg);
		//prnMsg($sql);bom.loccode=locstock.loccode	AND
		//echo $TableHeader;
		$RowCounter =0;
...
$Errors = array();
$InputError = 0;
$SQL="SELECT `confvalue` FROM `myconfig` WHERE confname='BOM' AND costitem IN (SELECT `code`FROM workcentres WHERE worktype=1)";
$Result = DB_query($SQL);
...
if (isset($Select)) { //Parent Stock Item selected so display BOM or edit Component
	...
	if (isset($SelectedParent) AND isset($_POST['Submit'])) {
		...
		if($_POST['AutoIssue']==1 AND isset($_POST['Component'])){
			$sql = "SELECT controlled FROM stockmaster WHERE stockid='" .  explode('~',$_POST['Component'])[0] . "'";
		...
		if ( explode('~',$_POST['Component'])[0] == $SelectedParent) {
			$InputError = 1;
		...
		//need to check not recursive BOM component of itself!
			...
			if (!CheckForRecursiveBOM ($SelectedParent, explode('~', $_POST['Component'])[0], $db)) {

				/*Now check to see that the component is not already on the BOM */
				$sql = "SELECT component
						FROM bom
						WHERE parent='".$SelectedParent."'
						AND component='" . explode('~', $_POST['Component'])[0] . "'
	...
	echo '<br /><div id="Report">
			<table class="selection">';
	echo '<tr>
			<th colspan="16"><div class="centre"><b>' . $SelectedParent .' - ' . $myrow[0] . ' ('. $MBdesc. ') </b>' . $StockImgLink . '</div></th>
		</tr>';
	...
	if(count($BOMTree) == 0) {
		...
		foreach($BOMTree as $BOMItem){
			...
			//prnMsg($UltimateParent.', '.$Parent.', '.$Component.', '.$Level);
    ...
    /* We do want to show the new component entry form in any case - it is a lot of work to get back to it otherwise if we need to add */
			...
			*/
			...
			if ($ParentMBflag=='A'){ /*Its an assembly */
				...
				$sql = "SELECT stockmaster.stockid,
						...
						FROM stockmaster INNER JOIN stockcategory
							ON stockmaster.categoryid = stockcategory.categoryid
						WHERE  stockmaster.categoryid IN (".$BOMarr['stockloccode'].")

					
						ORDER BY stockmaster.stockid";
							/*
						((stockcategory.stocktype='L' AND stockmaster.mbflag ='D')
						OR stockmaster.mbflag !='D')
						AND stockmaster.mbflag !='K'
						AND stockmaster.mbflag !='A'
						AND stockmaster.stockid != '".$SelectedParent."'
			...
			$result = DB_query($sql,$ErrMsg, $DbgMsg);
			echo'<input type="text" name="Component" value="'.$_POST['Component'].'" id="Component" placeholder="输入编码、名称关键词筛选，然后选择物料" autocomplete="off"  list="ComponentList"   maxlength="50" size="50"  onChange="inSelect(this, ComponentList.options,'.	"'".'The  code '."'".'+ this.value+ '."'".' doesnt exist'."'".')" /> 
					<datalist id="ComponentList"> ';
					$n=1;
					while ($row=DB_fetch_array($result )){
						$len=20-strlen($row['stockid']);
						echo '<option value="' . str_pad($row['stockid'].'~',10, '-', STR_PAD_RIGHT) . $row['description'] . '"label=' .  $n. '>';
						$n++;
					}
		...
		echo '<tr>
                ...
                <td><input type="text" required="required" size="5" name="Sequence" value="' . $_POST['Sequence'] . '" /> 可以输入小数点</td>
            </tr>';
		echo '<tr>
				<td>' . _('Location') . ': </td>
		...
		if (!isset($_POST['EffectiveTo']) OR $_POST['EffectiveTo']=='') {
			$_POST['EffectiveTo'] = Date($_SESSION['DefaultDateFormat'],Mktime(0,0,0,Date('m'),(Date('d')+30),Date('y')));
		}
