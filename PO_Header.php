...
if (isset($_POST['EnterLines']) OR isset($_POST['AllowRePrint'])) {
	...
	//prnMsg('196'.$_SESSION['PO' . $identifier]->TaxRate.'='.$_POST['TaxRate'].'-'.$_POST['StkLocation']);
	...
	$_SESSION['PO' . $identifier]->TaxCatID=explode('^',$_POST['TaxCat'])[0];
	$_SESSION['PO' . $identifier]->TaxRate=explode('^',$_POST['TaxCat'])[1];
	...
	//prnMsg('242'.$_SESSION['PO' . $identifier]->TaxRate.'='.$_POST['TaxRate']);
...
if (isset($_GET['ModifyOrderNumber'])) {
	...
	include('includes/PO_ReadIn_Order.inc');
}
...
if (isset($_POST['SearchSuppliers'])) {
	...
	if (mb_strlen($_POST['Keywords']) > 0) {
		...
		$SQL = "SELECT suppliers.supplierid,
						...
						AND  used>=0	AND userid='".$_SESSION['UserID']."'
...
if ((!isset($_POST['SearchSuppliers']) or $_POST['SearchSuppliers'] == '') AND (isset($_SESSION['PO' . $identifier]->SupplierID) AND $_SESSION['PO' . $identifier]->SupplierID != '')) {
	...
	$_POST['TaxGroupID'] = $_SESSION['PO' . $identifier]->TaxGroupID;
	$_POST['TaxRate']=$_SESSION['PO' . $identifier]->TaxRate;
	$_POST['TaxCatID']=$_SESSION['PO' . $identifier]->TaxCatID;
...
if (isset($_POST['Select'])) {
	...
	if (($AuthRow = DB_fetch_array($AuthResult) and $AuthRow['cancreate'] == 0)) {
		...
		//prnMsg('447-'.$_SESSION['PO' . $identifier]->TaxRate.'='.$_POST['TaxRate']);
		//$_SESSION['PO' . $identifier]->
   ...
   */
	...
	//	$_POST['TaxGroupID'] =$myrow['taxgroupid'];
		$_POST['TaxRate']=$myrow['taxrate'];
		$_POST['TaxCatID']=$myrow['taxcatid'];
		$_POST['SupplierContact']= $myrow['contcatname'];
...
if ($_SESSION['RequireSupplierSelection'] == 1 OR !isset($_SESSION['PO' . $identifier]->SupplierID) OR $_SESSION['PO' . $identifier]->SupplierID == '') {
	...
	echo'<td><input type="text" name="SupplierContact" value="" /></td>';
				/*
	...
	echo '
		...
		<tr>
			<td>银行账号:</td>
			<td><input type="text" name="SuppDelAdd6" size="41" maxlength="15" value="' . $_POST['SuppDelAdd6'] . '" /></td>
		</tr>
		<tr>
			<td>' . _('Phone') . ':</td>
			<td><input type="tel" name="SuppTel" pattern="[0-9+\-\s()]*" size="31" maxlength="30" value="' . $_SESSION['PO' . $identifier]->SuppTel . '" /></td>
		</tr>';
		$sql="SELECT `taxcatid`, `taxcatname` ,taxrate FROM `taxcategories` WHERE onorder=2 OR onorder=3 ";
	
		$result=DB_query($sql);
		echo '<tr>
		<td>税目:</td>
		<td><select name="TaxCat">';//taxrate
