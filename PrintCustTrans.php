...
/* PrintCustTrans.php
 * @Author: ChengJiang 
 * @Date: 2019-06-23 17:02:58 
 * @Last Modified by: mikey.zhaopeng
 * @Last Modified time: 2019-06-23 18:35:03
 */
...
if (isset($PrintPDF) and $PrintPDF!='' and isset($FromTransNo) and isset($InvOrCredit)and $FromTransNo!='') {
	...
	$PaperSize ='A5_Landscape';// 'A4_Landscape';
	...
	// retrieve the invoice details from the database to print 	notice that salesorder record must be present to print the invoice purging of sales orders will  nobble the invoice reprints 
	//从数据库中检索发票详细信息以打印通知，即必须存在销售订单记录才能打印销售订单的发票清除将 禁止重新打印发票
	...
	// gather the invoice data
		...
		if ($InvOrCredit=='Invoice') {
			$sql = "SELECT debtortrans.trandate,
						...
						LEFT JOIN unittag ON unittag.tagID = custbranch.tag						
		...
		} else {// then its a credit note 那么它是一张信用证
			$sql = "SELECT debtortrans.trandate,
							debtortrans.ovamount,
							debtortrans.ovdiscount,
							debtortrans.ovfreight,
		...
		if (DB_num_rows($result)==1) {
			...
			if (DB_num_rows($result)>0) {
				...
				while ($myrow2=DB_fetch_array($result)) {
					...
					$LeftOvers = $pdf->addTextWrap($Left_Margin,$YPos,30,$FontSize,$rw,'center',1);
					$LeftOvers = $pdf->addTextWrap($Left_Margin+30,$YPos,150,$FontSize,$myrow2['stockid'].' '.$TranslationRow['descriptiontranslation'].$myrow2['description'],'left',1);
					...
					if (DB_num_rows($TranslationResult)==1){ //there is a translation
						...
						$LeftOvers = $pdf->addTextWrap($Left_Margin+200,$YPos,50,$FontSize,$TranslationRow['descriptiontranslation'],'right',1);
					} else {
						$LeftOvers = $pdf->addTextWrap($Left_Margin+100,$YPos,251,$FontSize,$myrow2['description'],'right',1);
					}
                       */
					//$lines=1;
					/*
					while($LeftOvers!='') {
						$LeftOvers = $pdf->addTextWrap($Left_Margin+200,$YPos,50,$FontSize,$LeftOvers);
						$lines++;
					}
					*/
					$LeftOvers = $pdf->addTextWrap($Left_Margin+180,$YPos,30,$FontSize,$myrow2['units'],'centre',1);
					$LeftOvers = $pdf->addTextWrap($Left_Margin+210,$YPos,50,$FontSize,$DisplayQty,'right',1);
					$LeftOvers = $pdf->addTextWrap($Left_Margin+260,$YPos,60,$FontSize,$DisplayPrice,'right',1);
				
					$LeftOvers = $pdf->addTextWrap($Left_Margin+320,$YPos,80,$FontSize,$DisplayNet,'right',1);
					$LeftOvers = $pdf->addTextWrap($Left_Margin+400,$YPos,$Page_Width-$Right_Margin-$Left_Margin-400,$FontSize,$DisplayDiscount,'right',1);
			...
			$LeftOvers = $pdf->line($Left_Margin,$YPos-4,$Page_Width-$Right_Margin,$YPos-4);
			$YPos -= $line_height;
			$LeftOvers =$pdf->addText($XPos+50, $YPos,$FontSize,'验收人:' );
			$LeftOvers =$pdf->addText($XPos+200, $YPos,$FontSize,  '交物人:' );
			$LeftOvers =$pdf->addText($XPos+350, $YPos,$FontSize,  '制单人:'.$_SESSION['UsersRealName']);
              /*
	...
	if (isset($_GET['Email'])){ //email the invoice to address supplied
		...
		$FileName = $_SESSION['reports_dir'] . '/销售发货单_' . $InvOrCredit . '_' . $FromTransNo . '.pdf';
		...
		ob_end_clean();
		$pdf->OutputD( '销售发货单_'. $FromTransNo . '.pdf');
...
} else { //The option to print PDF was not hit 未点击打印PDF的选项
