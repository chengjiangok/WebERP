...
if (isset($_POST['submit']) AND isset($_POST['NewDatabase'])) {
	...
	if(mb_strlen($_POST['NewDatabase'])>32
		...
		if (!file_exists('./companies/' . $_POST['NewDatabase'])
			...
			if ($_POST['CreateDB']==TRUE){
               ...
               //  prnMsg($DBType);  
				if ($DBType=='postgres'){
					...
					$SQLScriptFile = file('./sql/pg/country_sql/default.sql');

				} elseif ($DBType =='mysql') { //its a mysql db < 4.1
					mysql_select_db($_POST['NewDatabase'],$db);
					$SQLScriptFile = file('./sql/mysql/country_sql/WebERPChina.sql');
			...
			$Result = mkdir('./companies/' . $_POST['NewDatabase'] . '/BankCopy');
			$Result = mkdir('./companies/' . $_POST['NewDatabase'] . '/TaxInvoice');
			$Result = mkdir('./companies/' . $_POST['NewDatabase'] . '/workhours');
			$Result = mkdir('./companies/' . $_POST['NewDatabase'] . '/pdf_append');
