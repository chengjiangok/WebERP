...
/* $Id: SelectCompletedOrder.php $*/
/*
 * @Author: ChengJiang 
 * @Date: 2018-10-26 17:06:24 
 * @Last Modified by:   ChengJiang 
 * @Last Modified time: 2018-10-26 17:06:24 
 */
...
if (isset($_POST['SearchParts']) AND $_POST['SearchParts']!=''){
	...
	if (isset($OrderNumber)) {
		$SQL = "SELECT salesorders.orderno,
						...
						SUM(salesorderdetails.curramount) AS currordervalue,
						SUM(salesorderdetails.amount) AS ordervalue
		...
		if (isset($SelectedCustomer) AND !isset($OrderNumber) AND !isset($CustomerRef)) {
			...
			if (isset($SelectedStockItem)) {
				$SQL = "SELECT salesorders.orderno,
								...
								salesorders.deliverto,
								salesorders.currcode,
						SUM(salesorderdetails.curamount) AS currordervalue,
								 SUM(salesorderdetails.amount) AS ordervalue
							FROM salesorders INNER JOIN salesorderdetails
								ON salesorders.orderno = salesorderdetails.orderno
								INNER JOIN debtorsmaster
...
echo '<tr>
		...
		<td><input type="text" name="CustomerRef" maxlength="8" size="9" /></td>
		<td></td>
		<td colspan="2">
			<input type="checkbox" ' . $ShowChecked . ' name="completed" />' . _('Show Completed orders only') . '</td>
		</tr>';

echo '</table>';

if (!isset($SelectedStockItem)) {
	$result1 = DB_query("SELECT categoryid,  categorydescription
				FROM stockcategory
				INNER JOIN locationusers ON locationusers.loccode = categoryid AND locationusers.userid = '".$_SESSION['UserID']."' AND locationusers.canupd = 1
				 WHERE stocktype='M' ORDER BY categorydescription");
...
/*show a table of the orders returned by the SQL */
	...
	while ($myrow=DB_fetch_array($SalesOrdersResult)) {
		...
		$FormatedCurrOrderValue = locale_number_format($myrow['currordervalue'],$myrow['currdecimalplaces']);
