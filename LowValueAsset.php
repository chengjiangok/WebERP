...
if (isset($_POST['submit'])) {
	...
	/*    foreach($_POST['chkbx'] as $value){
			...
			$sql="INSERT INTO lowvalueassete( 
				                             description, 
											 longdescription,
										     assetcategoryid,
			                                 assetlocation,
			...
			$valarr= explode('^',$value);
                ...
                $sql="UPDATE gltrans 
			          SET posted=0 
					  WHERE transno='".$valarr[2]."'  AND periodno='".$_SESSION['period']."'  AND  LEFT(account,4)='1601'";
				$result = DB_query($sql, $ErrMsg, $DbgMsg);
			}*/
			if (DB_error_no() ==0) {
...
if (isset($_POST['Description'])) {
	...
	echo	'</select>
	 ...
	 $sql="SELECT transno, amount*flg  amo FROM gltrans WHERE left(account,4)='1601' AND periodno=12";
...
echo '</select>
	...
	<tr>
		<td>保管人:</td>
		<td><input ' . (in_array('Custodian',$Errors) ?  'class="inputerror"' : '' ) .'  type="text" name="Custodian" size="20" maxlength="20" value="' . $_POST[Custodian] . '" /></td>
	</tr>
	<tr>
		<td>' . _('Bar Code') . ':</td>
...
if ($_POST['DepnType']==0){ //straight line
	echo '<option selected="selected" value="0">一次摊销法</option>';
	echo '<option value="1">五五摊销法</option>';
} else {
	echo '<option value="0">一次摊销法</option>';
	echo '<option selected="selected" value="1">五五摊销法</option>';
}
...
echo '</select></td>
      ...
      <tr>
			<td>录入方式</td>
				<td>
				    <input type="radio" name="query" value="0"  '.($_POST['query']==0 ? 'checked':"").' >盘盈          
          	
           			<input type="radio" name="query" value="1"   '.($_POST['query']==1 ? 'checked':"").' >购入 
                </td>
            </tr>
      <tr>
			<td>金额:</td>
			<td><input type="text" class="number" name="cost" size="20" maxlength="20" value="'.$_POST[cost].'" /></td>
		</tr>  
	</table>';
...
//	$sql="SELECT  transno FROM gltransimport WHERE tag=".$str[1]." AND  periodno='" .$_POST['selectprd']."'";
	...
	$sql="SELECT gltrans.typeno,
			...
			WHERE   gltrans.periodno='".$_SESSION['period']."' AND posted=1
			AND LEFT(gltrans.account,4)='1601' 		
	...
	echo '<td class="number">
	         <input type="checkbox" name="chkbx[]" value="'.$myrow['Debits'].'^'.$myrow['typeno'].'^'. $myrow['transno'].'^'.$myrow['trandate'].'" checked="true" ></td></tr>';
	$LastType = $myrow['type'];
	...
	echo '</table>';
		...
		prnMsg('没有明细器具需要录入！','info');
	}
	//end if no bank trans in the range to show   checked=tr
///
if (isset($AssetRow)){
	echo '<table>
		<tr>
...
if (isset($New)) {
	echo '<div class="centre">
			<br />
			<input type="submit" name="submit" value="新增器具" />';
} else {
	echo '<br />
		<div class="centre">
