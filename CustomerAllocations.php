...
$Title ='客户收款分配';// _('Customer Receipt') . '/' . _('Credit Note Allocations');
...
if (isset($_POST['AllocTrans'])) {
	...
	*/
	...
	if (DB_num_rows($result)==0) {
		prnMsg('492'._('No outstanding receipts or credits to be allocated for this customer'),'info');
		include('includes/footer.php');
		exit;
	}
    ...
    //(debtortrans.type=12 OR debtortrans.type=11)
	if ($_SESSION['SalesmanLogin'] != '') {
		$SQL .= " AND debtortrans.salesperson='" . $_SESSION['SalesmanLogin'] . "'";
	}
	...
	if ($NoOfUnallocatedTrans == 0) {
		prnMsg('617'._('There are no allocations to be done'),'info');
	} else {
		echo '<tr class="OddTableRows">
				<td colspan="7" class="number"><b>' . locale_number_format($Balance,$CurrDecimalPlaces)  . '</b></td>
				<td><b>' . $CurrCode . '</b></td>
