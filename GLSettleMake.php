...
if (DB_num_rows($Result)>0){
		echo '<table class="selection">';
		echo'<tr>
		<th colspan="6"   class="centre">'.substr($_POST['SelectDate'],0,4).'年产品成本结转</th>';
	...
	$SQL=" SELECT   gltrans.tag,
					...
					WHERE periodno <=".$SelectPrd."
						AND  gltrans.tag=".$_POST['UnitsTag']."
							AND   account like '5001%'
							group by  gltrans.tag, account,accountname";
	$Result = DB_query($SQL);			
	if (DB_num_rows($Result)>0){
		echo'
			<tr>
				<th colspan="6"><h3>结账期产品成本发生额</h3></th>
			<tr>
			<th >月份</th>
			<th >分组</th>				
			<th >科目编码/名称</th>
		...
		while ($row = DB_fetch_array($Result)) {
			...
			$AmoTotal+=$row['qmye'];
			if ($row['tag']>0){
				$ToActAmo[$row['account']][0]=$row['occur_de'];
				$ToActAmo[$row['account']][1]=$row['qmye'];
			}
	...
	echo '</table>
		...
		if ($SettleTab[6]>=1){
			...
			prnMsg(substr($_POST['SelectDate'],0,7).'产品成本已经结转!','info');
	...
	foreach($SettleRule as  $key=>$row){
		...
		if (strlen($CostActStr)>5){
			...
			echo'<td rowspan="'.$rw2.'">'.$RowIndex.'</td>			      
					...
					<input type="hidden"  name="Rmk'.$key.'" id="Rmk'.$key.'"   value="月末结转产成品"  />
				...
				if ($rw2>0){
					foreach($ToActArr as $keyAct=> $val){
						...
						if ($_POST[$key.'Edit'.$keyAct]==2){
							...
							$SettleRule[$key][6]=round($SettleRule[$key][0]/$rw2,2);
							$ToAmoTotal+=round($ToActAmo[$keyAct][0],2);
						}
						$_POST[$key.'ToAmo'.$keyAct]=$ToActAmo[$keyAct][0];
						...
						<td > <input type="text"  class="number" name="'.$key.'Remark'.$keyAct.'"   id="'.$key.'Remark'.$keyAct.'"  size="15"  value="'.$_POST[$key.'Remark'.$keyAct].'"   title="'.$msg.'"  pattern="[\w\d\u0391-\uFFE5\(\)\[\]\ +$"  placeholder="月末结转产品成本'.substr(strstr($val[1],'-'),1).'" onchange="OnRemark(this ,'.$key.')"  />	</td>';
						echo'</tr>';
						$tr='<tr>';
						$rw++;								
					}  
				...
				echo'<script type="text/javascript">
						document.getElementById("CostAmo'.$key.'").value='.$AmoTotal.'; 	
					</script>';	
		...
		if(isset($_POST['Updateing'])){
			...
			foreach ($_POST as $FormVariableName =>$Qty) {
				...
				if (round($AmoTotal,2)==0){
						...
						if ($InitialTab[6]==0){
							$SettleTab[6]=$SettleTab[6]+1;
						}elseif ($InitialTab[6]==-1){
							$SettleTab[6]=$SettleTab[6]+2;
						}
						$Status=0;
					...
					prnMsg('产品成本结转会计凭证: ' . $TransNo . ' ' . _('has been successfully entered'),'success');
...
echo '<div class="centre">
			<input type="submit" name="SettleIssue" value="结转产品成本" />	';
