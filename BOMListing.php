...
/*
 * @Author: ChengJiang 
 * @Date: 2019-06-10 06:05:23 
 * @Last Modified by: mikey.zhaopeng
 * @Last Modified time: 2019-06-10 06:12:51
 */
include('includes/session.php');
 ...
 $SQL = "SELECT bom.parent,
				...
				FROM stockmaster INNER JOIN bom ON stockmaster.stockid=bom.component
	...
	$SQL.=" ORDER BY bom.parent,bom.component";
    //prnMsg($SQL);
	$BOMResult = DB_query($SQL,'','',false,false); //dont do error trapping inside DB_query
	if (!isset($_POST['PrintPDF'])){ /*The option to print PDF was not hit */
		...
		/*if $FromCriteria is not set then show a form to allow input	*/
			...
			*/
			...
			echo '</table>
					<br /><div class="centre">
					<input tabindex="3" type="submit" name="Search" value="查询" />
	
					<input tabindex="3" type="submit" name="PrintPDF" value="' . _('Print PDF') . '" />
					</div>';
...
if (isset($_POST['PrintPDF'])){
	...
	$RowIndex = 0;
	if (DB_num_rows($BOMResult) <> 0) {
		DB_data_seek($BOMResult, ($_POST['PageOffset'] - 1) * $_SESSION['DisplayRecordsMax']);
	}
	while (($myrow = DB_fetch_array($BOMResult)) AND ($RowIndex <> $_SESSION['DisplayRecordsMax'])) {
		if ($k==1){
