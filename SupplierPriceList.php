...
/*
 * @Author: ChengJiang 
 * @Date: 2017-10-04 10:07:25 
 * @Last Modified by: ChengJiang
 * @Last Modified time: 2017-10-04 10:11:06
 */
...
if (isset($_POST['SearchSupplier'])) {
	...
	if (mb_strlen($_POST['Keywords']) > 0) {
		...
		$SQL = "SELECT suppliers.supplierid,
					...
					suppliers.address3
					FROM suppliers 				
				WHERE suppliers.supplierid IN (SELECT `csno` FROM `custsupusers` WHERE userid='admin' AND code='')
						AND suppliers.suppname " . LIKE  . " '".$SearchString."'";
		...
		$SQL = "SELECT suppliers.supplierid,
			...
			FROM suppliers
			WHERE suppliers.supplierid IN (SELECT `csno` FROM `custsupusers` WHERE userid='admin' AND code='')
					AND  suppliers.supplierid " . LIKE . " '%" . $_POST['SupplierCode'] . "%'";
