...
/*
 * @Author: ChengJiang 
 * @Date: 2018-10-22 06:13:14 
 * @Last Modified by: ChengJiang
 * @Last Modified time: 2018-10-22 06:13:36
 */
	...
	*/
	...
	if($InputErrors==0) {
		...
		if(DB_num_rows($result)==0) {
			...
			prnMsg( _('against customer code') . ': ' . $_POST['Select'] . ' ' . _('could not be retrieved') ,'error');
		...
		///	$_SESSION['Items'.$identifier]->SpecialInstructions ='';//
			...
			$_SESSION['Items'.$identifier]->SalesPerson = $myrow['contactname'];

			$_SESSION['Items'.$identifier]->CurrCode = $myrow['currcode'];
			$_SESSION['Items'.$identifier]->CatID = $myrow['taxcatid'];  //准备放弃
		...
		if($StockItem->MBflag=='M'
			...
			if($ShortfallQuantity < 0) {//then we need to make a work order
				...
				if(is_null($CostRow[0]) OR $CostRow[0]==0) {
					$Cost =0;
					//自动为'）.'创建工单''。$StockItem->StockID.''。_（'此销售订单上的项目，此项目的成本从组件成本之和中累计为零。这可能是因为没有建立物料清单。。。你可以再检查一遍
				...
				//prnMsg('559'.$FactoryManagerEmail);
...
if(in_array(2,$_SESSION['AllowedPageSecurityTokens'])) {
	...
	if($_SESSION['Items'.$identifier]->Quotation==1) {
		echo '<tr><th colspan="8">' . _('Quotation Summary') . '</th></tr>';
	} else {
		echo '<tr><th colspan="8">' . _('Order Summary') . '</th></tr>';
	}
	echo '<tr>
				<th>' .  _('Item Code')  . '</th>
	...
	$LineQtyTotal=0;
	foreach ($_SESSION['Items'.$identifier]->LineItems as $StockItem) {
		$LineTotal = $StockItem->Quantity * $StockItem->TaxPrice;
		$LineTaxTotal =locale_number_format( $LineTotal/ (1+ $StockItem->Cess)*$StockItem->Cess,2);
		$DisplayLineTotal = locale_number_format($LineTotal,$_SESSION['Items'.$identifier]->CurrDecimalPlaces);
		$DisplayPrice = locale_number_format($StockItem->TaxPrice,$_SESSION['Items'.$identifier]->CurrDecimalPlaces);
		...
		echo '<td>' . $StockItem->StockID . '</td>
			...
			<td class="number">' . (100*$_SESSION['Items'.$identifier]->TaxRate) . '</td>
		...
		$LineQtyTotal=$LineQtyTotal+$StockItem->Quantity ;
	}

	$DisplayTotal = number_format($_SESSION['Items'.$identifier]->total,2);
	echo '<tr class="EvenTableRows">
			<td colspan="2" class="number"><b>合计</b></td>
			<td class="number">' . $LineQtyTotal . '</td>
			<td colspan="3" ></td>
...
/*Display the order without discount */
	...
	foreach ($_SESSION['Items'.$identifier]->LineItems as $StockItem) {
		...
		echo '<td>' . $StockItem->ItemDescription  . '</td>
				...
				<td>' . $StockItem->Units . '</td>
				<td class="number">' . $DisplayPrice . '</td>
				<td class="number">'.($_SESSION['Items'.$identifier]->TaxRate*100) . '</td>
