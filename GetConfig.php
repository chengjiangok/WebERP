...
/* $Id: GetConfig.php 7800 2017-06-26 05:03:16Z exsonqu $*/
/*
 * @Author: ChengJiang 
 * @Date: 2018-03-04 14:04:16 
 * @Last Modified by: ChengJiang
 * @Last Modified time: 2018-05-19 05:59:42
 */
if(isset($ForceConfigReload) AND $ForceConfigReload==true OR !isset($_SESSION['CompanyDefaultsLoaded'])) {
	...
	while( $myrow = DB_fetch_array($ConfigResult) ) {
		...
		} elseif($myrow['confname']=='weberp') {
			$_SESSION[$myrow['confname']] = explode(',',substr($myrow['confvalue'],0,-1));
		} else {
			$_SESSION[$myrow['confname']] =  $myrow['confvalue'];
		}
	} //end loop through all config variables
	$_SESSION['CompanyDefaultsLoaded'] = true;
	$sql="SELECT periodno,lastdate_in_period FROM periods where date_format(lastdate_in_period,'%Y-%m')='".date("Y-m",strtotime($_SESSION['ProhibitPostingsBefore'])+86400)."'";
	
	$result = DB_query($sql);

	$row=DB_fetch_assoc($result,$db);
	
	$_SESSION['lastdate']=$row['lastdate_in_period'];
	$_SESSION['period']=$row['periodno'];
	//本年1月会计期间
	$_SESSION['janr']=$row['periodno']-date("m",strtotime($row['lastdate_in_period']))+1;	
	$_SESSION['CompanyDefaultsLoaded'] = true;
	...
	*/
	...
	if (DB_num_rows($ReadCoyResult)==0) {
		...
		while($row=DB_fetch_array($ReadCoyResult)){
			$_SESSION['CompanyRecord'][$row['coycode']]=$row;
		}
	}
	...
	if(!isset($_SESSION['tagref'])){
		$SQL="SELECT `tagref`, `tagdescription`, stamp,`tagsgroup`, `grouptype`,tagtype FROM `tags`  ORDER BY grouptype";
		$result=DB_query($SQL);
	
		while($row=DB_fetch_array($result)){
	  
			 $taggroup=explode(",",$row['tagsgroup']);
		 	if ($row['grouptype']==1||$row['tagtype']>=0){
		     //凭证编号组
				$_SESSION['tagsgroup'][$row['tagref']]=array($row['tagsgroup'],$row['stamp'],$row['tagdescription'],$row['grouptype']);
			 }
			if (!empty($row['stamp'])){ 
				foreach($taggroup as $val){
				
					$_SESSION['tagref'][$val]=array($row['tagref'],$row['tagsgroup'],$row['stamp'],$row['tagdescription']);
				}
			}
	  	}
	}
