	...
	for ($AllocCounter=0; $AllocCounter < $_POST['TotalNumberOfAllocs']; $AllocCounter++){
		...
		if (!is_numeric($_POST['Amt' . $AllocCounter])){
		 ...
		 if ($_POST['Amt' . $AllocCounter] < 0){
			prnMsg('60'._('The entry for the amount to allocate was negative') . '. ' . _('A positive allocation amount is expected'),'error');
			$_POST['Amt' . $AllocCounter] = 0;
		 }
...
If (isset($_GET['AllocTrans'])){
	...
	if (DB_num_rows($Result) != 1){
		prnMsg('319'._('There was a problem retrieving the information relating the transaction selected') . '. ' . _('Allocations are unable to proceed'), 'error');
		if ($debug == 1){
...
if (isset($_POST['AllocTrans'])){
  ...
  $sql = "SELECT id,
			...
			WHERE  settled=0
			ORDER BY id";
			//(supptrans.type=21 OR supptrans.type=22)
