...
if (isset($_GET['WONO'])) {
	$SelectedWO = explode('-',$_GET['WONO'])[1];
	$_POST['WO']=$SelectedWO;
	$_POST['WONO']=explode('-',$_GET['WONO'])[0];
	//$identifier = date('U');
	$_POST['WN']=$_GET['WONO'];
} elseif (isset($_POST['WN'])){
	$SelectedWO = explode('-',$_POST['WN'])[1];
	$_POST['WO']=$SelectedWO;
	$_POST['WONO']=explode('-',$_POST['WN'])[0];
} else {
	unset($SelectedWO);
}
...
$WOResult = DB_query("SELECT workorders.loccode,
							...
							closecomments
						FROM workorders
						INNER JOIN woitems	ON workorders.wono=woitems.wono
...
while ($WORow = DB_fetch_array($WOItemsResult)){
	 ...
	 echo '<tr>
				<td>' . $WORow['stockid'] . '</td>
	 			...
	 			<td class="number"><a href="'. $RootPath . '/WorkOrderReceive.php?WONO=' . $_POST['WONO'].'-'.$_POST['WO']. '&amp;StockID=' . $WORow['stockid'] . '">' . _('Receive') . '</a></td>
	 			<td class="number"><a href="'. $RootPath . '/WorkOrderIssue.php?WONO=' . $_POST['WONO'].'-'.$_POST['WO'] . '&amp;StockID=' . $WORow['stockid'] . '">' . _('Issue') . '</a></td>
 			</tr>';
...
$SQL="SELECT worequirements.stockid,
			...
			worequirements.stdcost";
		//	prnMsg($SQL);
$RequirementsResult = DB_query($SQL);
...
If (isset($_POST['Close'])) {
	...
	$TotalCostVar += $CostVar;
	$TotalUsageVar += ($WOIssuesRow['qty']*$WOIssuesRow['standardcost']);
   //$TotalVariance*$ShareProportion*(1-$ProportionOnHand)数量
   //$TotalStdValueRecd;合计领料成本
   $ShareProportion  （收货数*单位成本）/合计领料成本
   */
	while ($WORow = DB_fetch_array($WOItemsResult)){
